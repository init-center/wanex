...
export const feedbackMatrixScenario = createEvalScenario({
  id: "assistant.app-feedback-matrix-contract",
  title: "assistant feedback matrix aligns Web and TUI blocked/succeeded outcomes",
  tags: [
    "assistant",
    "web",
    "tui",
  ...
  async run(context) {
    const storage = await createConversationSettlementFixture({
      ...
      prefix: "wanex-eval-assistant-feedback-"
    })
    ...
    try {
      ...
      const blockedTui = await runTuiLineSession({
        ...
        input: lines([
          ...
          "preview assistant.agent.submit {\"text\":\"tui preview should report blocked provider\"}",
          "execute assistant.agent.submit {\"text\":\"tui execution should report blocked provider\"}",
          "quit"
        ]),
        write(chunk) {
      ...
      const readyTui = await runTuiLineSession({
        ...
        input: lines([
          ...
          "preview assistant.agent.submit {\"text\":\"tui preview should report runnable provider\"}",
