...
import { validateProductAppBackendCommandSchemaInput } from "../../src/backend/product-command-schema-validator.js"

describe("product command schema validator", () => {
  it("validates nested supported constraints with Unicode code-point length", () => {
    ...
    expect(
      validateProductAppBackendCommandSchemaInput(schema, {
        label: "😀",
        mode: "safe",
        ratio: 0.5,
        nested: { enabled: true }
  ...
  it("treats defaults as annotations and never injects missing values", () => {
    ...
    expect(validateProductAppBackendCommandSchemaInput(schema, input)).toEqual([
  ...
  it("rejects cycles, accessors, sparse arrays, non-finite values, and excess issues", () => {
    ...
    cyclic.self = cyclic
    expect(validateProductAppBackendCommandSchemaInput(openSchema, cyclic)).toEqual([
    ...
    expect(validateProductAppBackendCommandSchemaInput(openSchema, accessor)).toEqual([
    ...
    const issues = validateProductAppBackendCommandSchemaInput(
