...
describe("@wanex/product", () => {
  it("opens a product backend shell without exposing storage internals", async () => {
    ...
    try {
      ...
      expect(home.product.provider.activeEndpointId).toBe("product-test")
  ...
  it("projects provider readiness without exposing provider secrets", async () => {
    ...
    const missingCredentialApp = await createTestApp(
      missingCredentialStoreDir,
      {
        modelEndpoint: productTestModelEndpoint({
          endpointId: "product-openai-missing-key",
          protocolId: "openai-chat-completions",
          providerId: "openai-compatible",
          modelId: "product-openai-missing-key-model"
        })
      }
    )
    try {
      ...
      expect(home.providerReadiness).toMatchObject({
        ...
        activeEndpointId: "product-openai-missing-key",
        endpointCount: 1,
        canRun: false,
        attentionRequired: true,
    ...
    const readyApp = await createTestApp(readyStoreDir, {
      modelEndpoint: productTestModelEndpoint({
        endpointId: "product-openai-ready",
        protocolId: "openai-chat-completions",
        providerId: "openai-compatible",
        modelId: "product-openai-ready-model",
    ...
    try {
      ...
      expect(home.providerReadiness).toMatchObject({
        status: "ready",
        reason: "active_endpoint_ready",
        activeEndpointId: "product-openai-ready",
        endpointCount: 1,
        canRun: true,
        attentionRequired: false,
        requiresCredential: true,
  ...
  it("previews product command invocation through the product run gate", async () => {
    const readyStoreDir = await createStoreDir()
    const readyApp = await createTestApp(readyStoreDir)
    try {
    ...
    const blockedApp = await createTestApp(blockedStoreDir, {
      modelEndpoint: productTestModelEndpoint({
        endpointId: "product-preview-blocked-provider",
        protocolId: "openai-chat-completions",
        providerId: "openai-compatible",
        modelId: "product-preview-blocked-model"
      })
    })
    try {
      ...
      expect(blocked).toMatchObject({
        ...
        providerReadiness: {
          status: "missing_required_credential",
          canRun: false,
          activeEndpointId: "product-preview-blocked-provider"
        }
      })
  ...
  it("fails closed for conversation submits when the provider cannot run", async () => {
    ...
    const app = await createTestApp(storeDir, {
      modelEndpoint: productTestModelEndpoint({
        endpointId: "product-blocked-provider",
        protocolId: "openai-chat-completions",
        providerId: "openai-compatible",
        modelId: "product-blocked-model"
      })
    })
    try {
      ...
      expect(started).toEqual({
        ...
        reason: "provider_not_ready",
        message:
          "provider is not ready: active model endpoint product-blocked-provider is missing a required credential"
      })
      ...
      expect(typedPaletteRun).toMatchObject({
        ...
        commandId: "product.agent.submit",
        reason: "provider_not_ready",
        handlerRef: "wanex.product.backend.submitConversationOperation",
        providerReadiness: {
          canRun: false
        }
      })
  ...
  it("exposes a safe app-owned settings profile read model", async () => {
    ...
    try {
      ...
      expect(settings).toMatchObject({
        ...
        profile: {
          activeModelEndpointId: "product-test"
        },
        renderer: {
  ...
  it("keeps selected session, layout, mode, and preferences as app-owned state", async () => {
    const storeDir = await createStoreDir()
    await seedSession(storeDir, "ses_product_app_state")
    const app = await createTestApp(storeDir, {
      state: {
        layout: "split",
        mode: "workbench",
    ...
    try {
      ...
      expect(app.status().state).toEqual({
        selection: {
          kind: "session",
          sessionId: "ses_product_app_state"
        },
        layout: "diagnostics",
        mode: "diagnostics",
        preferences: {
  ...
  it("loads product state from an injected app-owned state store", async () => {
    const storeDir = await createStoreDir()
    const stateStore = createMemoryStateStore({
      selection: {
        kind: "session",
        sessionId: "ses_stored_product_app"
      },
      layout: "split",
      mode: "workbench",
      preferences: {
  ...
  it("lets explicit initial product state override stored state", async () => {
    const storeDir = await createStoreDir()
    const stateStore = createMemoryStateStore({
      selection: {
        kind: "session",
  ...
  it("persists each product state mutation before reporting success", async () => {
    const storeDir = await createStoreDir()
    await seedSession(storeDir, "ses_saved_product_app")
    const stateStore = createMemoryStateStore()
    const app = await createTestApp(storeDir, { stateStore })
    try {
      ...
      expect(saved).toEqual({
        selection: {
          kind: "session",
          sessionId: "ses_saved_product_app"
        },
        layout: "split",
        mode: "workbench",
        preferences: {
  ...
  it("fails closed when the product state store rejects a mutation", async () => {
    const storeDir = await createStoreDir()
  ...
  it("uses the selected session for asynchronous conversation submits", async () => {
    ...
    try {
      ...
      if (transcript.kind !== "product.session-transcript.found") {
      ...
      if (latestPage.kind !== "product.session-transcript.found") {
      ...
      expect(latestPage.transcript.rows.map(conversationRowText)).toEqual([
        ...
        "Fake response from product-test-model"
      ])
      ...
      if (earlierPage.kind !== "product.session-transcript.found") {
      ...
      await expect(app.readSessionTranscript({
        sessionId: "ses_product_cursor_other",
        cursor,
        limit: 1
      ...
      if (refreshed.kind !== "product.workbench.opened") {
  ...
  it("starts a conversation without a preselected session", async () => {
    ...
    try {
      ...
      if (started.kind !== "product.conversation-operation.found") {
      ...
      if (continued.kind !== "product.conversation-operation.found") {
  ...
  it("persists one opaque guided follow-up and promotes it after parent settlement", async () => {
    const storeDir = await createStoreDir()
    const sessionId = "ses_product_guided_follow_up"
    ...
    try {
      ...
      const submitted = await storage.submitSessionTurn({
        id: "inp_product_guided_parent",
        turnId: "turn_product_guided_parent",
        sessionId,
        jobId: "job_product_guided_parent",
        principalId: "principal_product_guided",
        idempotencyKey: "product-guided-parent-input",
        jobIdempotencyKey: "product-guided-parent-job",
        content: [
          {
            type: "text",
            id: "product_guided_parent_text",
            text: "complete the current task"
          }
        ],
        executionBinding: createTestTurnExecutionBinding(
          productTestModelEndpoint({
            endpointId: "product-test",
            modelId: "product-test-model"
          })
        ),
        maxSteps: 1
      })
      const workerId = "worker_product_guided_parent"
      ...
      if (
        queued.kind !== "product.conversation-operation.found" ||
      ...
      const invocation = await storage.beginProviderInvocation({
        ...
        step: 1,
        invocationNumber: 1,
        requestDigest: "product-guided-parent-provider"
      })
      await storage.settleSessionTurn({
        ...
        assistantMessage: [
          {
            type: "text",
            id: "product_guided_parent_answer",
  ...
  it("admits one opaque current-Turn guidance and reconstructs its durable pending state", async () => {
    const storeDir = await createStoreDir()
    const sessionId = "ses_product_steering"
    const storage = createStorageTestStore({
      kind: "local-system-service",
      mode: "oneshot",
      storeDir,
    ...
    try {
      ...
      const submitted = await storage.submitSessionTurn({
        id: "inp_product_steering",
        turnId: "turn_product_steering",
        sessionId,
        jobId: "job_product_steering",
        principalId: "principal_product_steering",
        idempotencyKey: "product-steering-input",
        jobIdempotencyKey: "product-steering-job",
        content: [
          {
            type: "text",
            id: "part_product_steering_input",
      ...
      const workerId = "worker_product_steering"
      const claimed = await storage.claimJob({
        workerId,
        leaseMs: 60_000,
        kinds: ["session.turn"]
            ...
            pending: [
              {
                steeringId: expect.stringMatching(
                  /^product_conversation_steering_/
                ),
                text: "focus the answer on failure handling",
                textTruncated: false
              }
      ...
      expect(controls[0]).toMatchObject({
        ...
        idempotencyKey: `product:steer:${expectedIdempotencyDigest}`,
        status: "pending",
        origin: {
          kind: "interactive",
          sourceRef: "product.steer",
  ...
  it("renames, archives, and restores sessions while reconciling selection", async () => {
    const storeDir = await createStoreDir()
    const app = await createTestApp(storeDir)
    const sessionId = "ses_product_lifecycle"
    try {
      ...
      const row = initial.product.sessions.recent.find(
      ...
      expect(
        archivedHome.product.sessions.recent.some(
          (session) => session.sessionId === sessionId
        )
      ).toBe(false)
      expect(archivedHome.product.sessions.archived).toContainEqual(
        expect.objectContaining({
          sessionId,
          title: "Renamed product chat",
          revision: 3
      ...
      expect(restoredHome.product.sessions.recent).toContainEqual(
  ...
  it("exposes a transport-neutral surface descriptor and safe command envelopes", async () => {
    const storeDir = await createStoreDir()
    await seedSession(storeDir, "ses_product_app_surface")
    const app = await createTestApp(storeDir)
    try {
      ...
      expect(selected).toMatchObject({
        ...
        event: {
          ...
          state: {
            selection: {
              kind: "session",
              sessionId: "ses_product_app_surface"
            }
          }
        }
      })
      ...
      expect(newConversation).toMatchObject({
        ...
        command: "startNewConversation",
        value: { mode: "chat" },
        event: { type: "product.surface.command_completed" }
      })
  ...
  it("enforces the provider run gate through the surface client boundary", async () => {
    ...
    const app = await createTestApp(storeDir, {
      modelEndpoint: productTestModelEndpoint({
        endpointId: "product-surface-blocked-provider",
        protocolId: "openai-chat-completions",
        providerId: "openai-compatible",
        modelId: "product-surface-blocked-model"
      })
    })
    try {
  ...
  it("prepares, removes, and safely projects conversation attachment drafts", async () => {
    const storeDir = await createStoreDir()
    await seedSession(storeDir, "ses_product_attachment_draft")
    const app = await createTestApp(storeDir, {
      modelEndpoint: productTestModelEndpoint({
        endpointId: "product-attachment",
        modelId: "product-attachment-model",
    ...
    try {
      ...
      await app.selectSession({ sessionId: "ses_product_attachment_draft" })
  ...
  it("submits canonical resource references, clears admitted drafts, and regenerates after restart", async () => {
    ...
    const options = {
      stateStore,
      modelEndpoint: productTestModelEndpoint({
        endpointId: "product-resource-submit",
        modelId: "product-resource-submit-model",
        inputModalities: ["text", "image"]
      })
    }
    ...
    try {
      ...
      expect(submitted.kind).toBe("product.conversation-operation.found")
      if (submitted.kind !== "product.conversation-operation.found") return
    ...
    try {
      ...
      expect(regenerated.kind).toBe("product.conversation-operation.found")
      expect(
  ...
  it("reviews Product recovery with opaque identities and disables retry for non-idempotent tools", async () => {
    const storeDir = await createStoreDir()
    const secretRef = "env://WANEX_PRODUCT_RECOVERY_KEY"
    const secretValue = "wanex-product-recovery-secret"
    let providerCalls = 0
    let toolCalls = 0
    ...
    globalThis.fetch = (async () => {
      ...
      return providerCalls === 1
        ? openAIToolCallResponse(
            "ambiguous_product_remote",
            "call_product_recovery",
            { remoteSecret: "product-input-must-not-escape" }
          )
        : openAIResponse("product recovery completed")
    }) as unknown as typeof globalThis.fetch
    const tools = new ToolRegistry()
    tools.register({
      name: "ambiguous_product_remote",
      ...
      annotations: { title: "Product ambiguous remote operation" },
      runtimeBinding: createToolRuntimeBinding({
        implementationId: "wanex.test.product.ambiguous-remote",
      ...
      async invoke(invocation) {
        ...
        return {
          ...
          message: "remote product response was lost",
          reconciliationRef: "remote-product-operation-1"
        }
      }
    ...
    const app = await createShell({
      ...
      artifacts: { explicitPath: serviceBin },
      modelEndpoint: productTestModelEndpoint({
        endpointId: "product-recovery-provider",
        protocolId: "openai-chat-completions",
        providerId: "openai-compatible",
        modelId: "product-recovery-model",
        baseUrl: "https://provider.product-recovery.example/v1",
        secretRef
      }),
      secretResolver: new SecretResolver([
        new EnvSecretProvider({ WANEX_PRODUCT_RECOVERY_KEY: secretValue })
      ]),
      runtimeContext: {
        tools,
    ...
    try {
      const submitted = await app.submitConversationOperation({
        ...
        sessionId: "ses_product_recovery_review"
      })
      expect(submitted.kind).toBe("product.conversation-operation.found")
      const recovery = await eventuallyProduct(async () => {
      ...
      if (
        recovery.kind !== "product.conversation-operation.found" ||
        recovery.operation.recovery === undefined
      ) {
        throw new Error("expected Product recovery review")
      }
      const item = recovery.operation.recovery.items[0]
      if (item === undefined) throw new Error("expected Product recovery item")
      ...
      expect(rendererJson).not.toContain("product-input-must-not-escape")
      expect(rendererJson).not.toContain("remote-product-operation-1")
      ...
      const resolved = await app.resolveTrackedConversationRecovery({
        ...
        content: [{
          type: "json",
          value: { remoteOperationId: "remote-product-operation-1" }
        }]
      })
      expect(resolved).toMatchObject({
        kind: "product.conversation-recovery.resolved",
        decision: "confirm_succeeded",
        operation: {
          kind: "product.conversation-operation.found",
          operation: {
      ...
      await waitForProductConversation(app, "ses_product_recovery_review")
  ...
  it("drives the application surface through the renderer-side client contract", async () => {
    ...
    try {
      ...
      if (
        !started.ok ||
        started.value.kind !== "product.conversation-operation.found"
      ) {
  ...
  it("exposes a host endpoint for renderer-like message clients", async () => {
    const storeDir = await createStoreDir()
    await seedSession(storeDir, "ses_product_app_host_endpoint")
    const app = await createTestApp(storeDir)
    try {
...
async function createStoreDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), "wanex-product-test-"))
  tempDirs.push(dir)
  return dir
}

async function eventuallyProduct<T>(run: () => Promise<T>): Promise<T> {
