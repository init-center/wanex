...
export async function resolveTrackedConversationApproval(request: {
  ...
  const receipt =
    await request.backend.commands.resolveConversationOperationApproval({
      ...
      idempotencyKey:
        ...
        assistantApprovalDecisionIdempotencyKey(
  ...
  return {
    kind: "assistant.conversation-approval.resolved",
...
function assistantApprovalDecisionIdempotencyKey(
  operationId: string,
  ...
  const digest = createHash("sha256")
    ...
    .digest("hex");
  return `assistant:approval:${digest}`;
}
