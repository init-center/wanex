...
describe("web application declarative command input", () => {
  ...
  it("projects only commands explicitly visible in an ordinary palette", () => {
    const result: Parameters<typeof projectCommandPalette>[0] = {
      ...
      value: {
        commands: [
          commandRow("assistant.status", "visible"),
          commandRow("assistant.shutdown", "hidden")
        ],
        diagnostics: []
      },
      event: {
    ...
    expect(projectCommandPalette(result)).toMatchObject({
      ...
      rows: [{ id: "assistant.status", input: { mode: "none" } }]
    })
  })
})
...
function commandRow(
  ...
  return {
    id,
    name: id.replace("assistant.", ""),
    title: id,
    handlerRef: `wanex.assistant.backend.${id}`,
    sourceKind: "builtin" as const,
    sourceScope: "builtin" as const,
    sourceId: "assistant.backend",
