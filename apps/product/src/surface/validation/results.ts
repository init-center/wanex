...
export function isSurfaceCommandValue(
  ...
  switch (command) {
    ...
    case "dismissSideQuery":
      return (
        isRecord(value) &&
        value.kind === "product.side-query.dismissed" &&
    ...
    case "dismissPlanGeneration":
      return (
        isRecord(value) &&
        value.kind === "product.plan-generation.dismissed" &&
    ...
    case "listPlanProposals":
      return (
        isRecord(value) &&
        value.kind === "product.plan-proposal-list" &&
...
function isPlanGenerationReadResult(value: unknown): boolean {
  if (!isRecord(value)) return false;
  if (value.kind === "product.plan-generation.missing") {
...
function isPlanProposalReadResult(value: unknown): boolean {
  if (!isRecord(value)) return false;
  if (value.kind === "product.plan-proposal.no-selection") return true;
  if (value.kind === "product.plan-proposal.missing") {
...
function isPlanProposalReadModel(value: unknown): boolean {
  return (
    isRecord(value) &&
    value.kind === "product.plan-proposal" &&
...
function isSideQueryReadResult(value: unknown): boolean {
  if (!isRecord(value)) return false;
  if (value.kind === "product.side-query.missing") {
    return typeof value.queryId === "string";
  }
  return (
    value.kind === "product.side-query.found" &&
