...
describe("Assistant Host storage-backed state store", () => {
  it("persists assistant state through the trusted local storage boundary", async () => {
    ...
    await first.save({
      ui: {
        ...
        selectedPlanProposalId: "plan_storage_assistant_app",
        layout: "split",
        mode: "workbench",
        preferences: {
          theme: "dark",
      ...
      pendingGuidedFollowUps: {
        ses_storage_assistant_app: {
          sessionId: "ses_storage_assistant_app",
          inputId: "input_storage_assistant_app_pending",
          turnId: "turn_storage_assistant_app_pending",
          jobId: "job_storage_assistant_app_pending"
        }
      },
  ...
  it("fails closed on malformed persisted assistant state", async () => {
    const storage = await createStorage()
...
async function createStorage(): Promise<StorageTestStore> {
  const storeDir = await mkdtemp(join(tmpdir(), "wanex-assistant-state-"))
