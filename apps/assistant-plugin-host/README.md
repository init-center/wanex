...
a narrow Assistant creation binding and an independently owned worker lifecycle;
it does not create an Assistant Shell, Surface, SurfaceClient, or second
application lifecycle.

## Use when

Use this entry when a assistant intentionally enables plugin-backed commands and
...
The returned trusted handle exposes only `assistantBinding`, optional bounded
`management`, `refresh()`, bounded status, and lifecycle methods. A trusted
upper composition passes
`assistantBinding` into its single Assistant Shell creation. The handle does not
expose storage, Plugin Runtime, the worker, subprocess hosts, install paths,
grants, payloads, or secrets. The caller owns Assistant and the injected storage
handle.

## Trusted Local Management

`CreateAssistantPluginHostOptions.management` is optional and trusted-host-only.
...
and the previous Assistant catalog remains active. Identical reconciliation emits
...
replacement removes old versions from new Assistant resolution while preserving
...
The Host publishes through its Assistant binding's Extension source. Assistant
projects a successful changed revision into its existing bounded Surface event
log; Web and TUI then reread `readAssistantCommands()`. The Host does not publish
renderer deltas or own a second event bus. Failed refresh keeps the previous
generation and emits no Assistant invalidation, while an identical refresh is a
silent no-op.

## Lifecycle

Create the host after opening Storage, create exactly one Assistant Shell with
`assistantBinding`, then call `start()` for background work or `runOnce()` for
...
never disposes Assistant or Storage. Zero-Plugin startup is valid.

For the standard local lifecycle, pass `createAssistantPluginComposition(...)`
as `StartAssistantHostOptions.pluginComposition`. Its structural contract is
...
it before Assistant and Storage disposal. Neither package depends on the other;
the trusted Assistant leaf selects and connects both.
