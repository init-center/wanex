...
describe("@wanex/local-host CLI options", () => {
  ...
  it("defaults to local profile storage and system-service artifact path", () => {
    expect(
      ...
      storage: {
        kind: "profile",
        rootDir: resolve("/workspace/product/.wanex-local-host"),
        profileId: "default"
      },
      modelEndpoints: {
        endpoints: []
  ...
  it("parses profile storage from environment", () => {
    expect(
      parseLocalCliOptions({
        ...
        args: [],
        env: {
          WANEX_LOCAL_HOST_HOSTNAME: "127.0.0.2",
          WANEX_LOCAL_HOST_PORT: "57016",
          WANEX_LOCAL_HOST_PROFILE_ROOT: "./profiles",
          WANEX_LOCAL_HOST_PROFILE_ID: "work",
          WANEX_SYSTEM_SERVICE_BIN: "./target/env-system-service",
          WANEX_LOCAL_HOST_OPEN: "yes",
          WANEX_LOCAL_HOST_SMOKE: "true",
          WANEX_LOCAL_HOST_SUMMARY_FORMAT: "json",
  ...
  it("parses product-local model endpoint flags over environment fallbacks", () => {
    expect(
      parseLocalCliOptions({
        cwd: "/repo",
        artifactRoot: "/repo",
        ...
        env: {
          WANEX_LOCAL_HOST_MODEL_ENDPOINT_ID: "env-endpoint",
          WANEX_LOCAL_HOST_PROVIDER_PROTOCOL: "fake",
          WANEX_LOCAL_HOST_PROVIDER_ID: "env-provider",
          WANEX_LOCAL_HOST_PROVIDER_MODEL_ID: "env-model",
          WANEX_LOCAL_HOST_PROVIDER_BASE_URL: "https://env.example.test/v1",
  ...
  it("parses explicit provider setup options", () => {
    ...
    expect(
      parseLocalCliOptions({
        ...
        args: [],
        env: {
          WANEX_LOCAL_HOST_SETUP_PROVIDER: "0"
        }
      }).setupProvider
    ).toBe(false)
    expect(() =>
  ...
  it("parses startup summary format options", () => {
    ...
    expect(
      parseLocalCliOptions({
        ...
        args: [],
        env: {
          WANEX_LOCAL_HOST_SUMMARY_FORMAT: "text"
        }
      }).summaryFormat
    ).toBe("text")
  ...
  it("parses product-local model endpoint environment over generic fallbacks", () => {
    expect(
      parseLocalCliOptions({
        cwd: "/repo",
        artifactRoot: "/repo",
        args: [],
        env: {
          WANEX_LOCAL_HOST_MODEL_ENDPOINT_ID: "local-env-endpoint",
          WANEX_LOCAL_HOST_PROVIDER_PROTOCOL: "openai-chat-completions",
          WANEX_LOCAL_HOST_PROVIDER_ID: "local-env-provider",
          WANEX_LOCAL_HOST_PROVIDER_MODEL_ID: "local-env-model",
          WANEX_LOCAL_HOST_MODEL_INPUT_MODALITIES: "text,image",
          WANEX_LOCAL_HOST_MODEL_OUTPUT_MODALITIES: "text",
          WANEX_LOCAL_HOST_PROVIDER_BASE_URL: "https://local.example.test/v1",
          WANEX_LOCAL_HOST_PROVIDER_SECRET_REF: "env://LOCAL_API_KEY",
  ...
  it("parses trusted model endpoint catalog files with secret refs", async () => {
    const dir = await tempDir("wanex-local-host-endpoint-catalog-")
  ...
  it("resolves explicit service binary paths from cwd", () => {
    expect(
      parseLocalCliOptions({
        cwd: "/workspace/wanex/apps/local-host",
        artifactRoot: "/workspace/wanex",
