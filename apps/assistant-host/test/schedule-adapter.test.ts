...
describe("Local Schedule adapter", () => {
  ...
  it("composes one durable Assistant Schedule port without adding Surface commands", async () => {
    const storeDir = await createStoreDir()
    const first = await startAssistantHost({
    ...
    const created = await first.schedules.createDefinition({
      definition: {
        prompt: "Persist through Assistant composition",
        trigger: { kind: "once", at: 42_000 },
      },
      idempotencyKey: "assistant-composition",
    })
    const definition = requireDefinition(created)
    ...
    const reopened = await startAssistantHost({
      storage: { kind: "store-dir", storeDir },
      serviceBin,
    })
    hosts.push(reopened)
