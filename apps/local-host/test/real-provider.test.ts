...
describe("@wanex/local-host real provider", () => {
  it("projects an exact Team pass without creating a public reply", async () => {
    ...
    const app = await startRealProviderApp({
      ...
      env: { WANEX_PRODUCT_REAL_PROVIDER_KEY: secretValue }
    })
  ...
  it("continues from first-run provider setup into the first real conversation", async () => {
    ...
    const storeDir = await createTempDir("wanex-product-local-first-run-")
    ...
    const submitted = await postJson(
      `${app.url}/wanex/web/request`,
      {
        ...
        requestId: "product_local_first_run_turn",
        action: {
  ...
  it("completes a chat-first Web turn through an environment-backed provider", async () => {
    ...
    const initialHtml = await fetchText(`${app.url}/`)
    const response = await postJson(`${app.url}/wanex/web/request`, {
      kind: "web.request",
      operation: "dispatchAction",
      ...
      endpoints: [
        {
          ...
          model: {
            ...
            catalog: {
              source: "custom",
              catalogId: "local-host.real-provider-test",
              revision: "1"
            }
          }
        }
      ],
      activeEndpointId: "product-local-real-provider"
    },
