...
describe("Product command catalog events", () => {
  it("invalidates once per changed revision and requires a canonical reread", async () => {
    const storeDir = await mkdtemp(join(tmpdir(), "wanex-product-catalog-event-"))
    ...
    const surface = createSurfaceAdapter(shell, {
      now: () => 41_000,
      streamId: "product-command-catalog-events"
    })
    ...
    try {
      ...
      const initial = await client.readProductCommands()
      ...
      const refreshed = await client.readProductCommands()
