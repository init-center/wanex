...
describe("Product ephemeral side-query coordinator", () => {
  ...
  it("wires the transient lifecycle through Product shell without state persistence", async () => {
    const storeDir = await createStoreDir()
    await seedSession(storeDir, "ses_product_side_query")
    const stateStore = createMemoryStateStore({
      selection: { kind: "session", sessionId: "ses_product_side_query" }
    })
    const app = await createShell({
      ...
      modelEndpoint: productTestModelEndpoint({
        endpointId: "product-side-query-provider",
        modelId: "product-side-query-model"
      }),
      stateStore
    })
    ...
    try {
      const running = await app.startSideQuery({
        question: "temporary product question"
      })
      expect(running.state).toBe("running")
      const succeeded = await readShellTerminal(app, running.queryId)
      expect(succeeded).toMatchObject({
        sessionId: "ses_product_side_query",
        modelEndpointId: "product-side-query-provider",
        state: "succeeded",
        answerText: "Fake response from product-side-query-model"
      })
      const persisted = JSON.stringify(stateStore.snapshot())
      expect(persisted).not.toContain(running.queryId)
      expect(persisted).not.toContain("temporary product question")
  ...
  it("carries typed commands and bounded invalidations through one Surface stream", async () => {
    const storeDir = await createStoreDir()
    await seedSession(storeDir, "ses_product_side_query_surface")
    const app = await createShell({
      storage: {
        kind: "local-system-service",
        storeDir
      ...
      artifacts: { explicitPath: serviceBin },
      modelEndpoint: productTestModelEndpoint({
        endpointId: "product-side-query-surface-provider",
        modelId: "product-side-query-surface-model"
      }),
      state: {
        selection: {
          kind: "session",
          sessionId: "ses_product_side_query_surface"
        }
      }
    })
    ...
    try {
      ...
      expect(started).toMatchObject({
        ...
        value: {
          state: "running",
          sessionId: "ses_product_side_query_surface",
          modelEndpointId: "product-side-query-surface-provider"
        },
        event: {
      ...
      expect(succeeded).toMatchObject({
        state: "succeeded",
        answerText: "Fake response from product-side-query-surface-model"
      })
      ...
      const invalidations = page.events.filter(
        (event) => event.type === "product.surface.side-query.invalidated"
      )
    ...
    backend: {
      commands: {
        ...
        async listModelEndpoints() {
          const endpoint = productTestModelEndpoint({
...
async function readShellTerminal(
  ...
  throw new Error(`Product shell side query did not settle: ${queryId}`)
}
...
async function readClientTerminal(
  ...
  for (let attempt = 0; attempt < 100; attempt += 1) {
    ...
    if (
      result.ok &&
      result.value.kind === "product.side-query.found" &&
...
async function createStoreDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), "wanex-product-side-query-"))
  tempDirs.push(dir)
  return dir
}
