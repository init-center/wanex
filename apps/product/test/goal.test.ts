...
import { createState } from "../src/state/product.js"
import {
  isSurfaceEnvelope,
  isSurfaceEvent
...
describe("@wanex/product Goal journey", () => {
  it("requires an active selected Session, criteria, and a runnable Provider", async () => {
    ...
    try {
      ...
      await expect(goal.start(goalStartRequest())).resolves.toMatchObject({
        kind: "product.goal",
        sessionId: "ses_goal",
        objective: "Finish the Product Goal journey",
        state: "active"
      })
    } finally {
      await goal.dispose()
  ...
  it("rejects malformed Goal Surface input, values, and event payloads", () => {
    ...
    const event = {
      id: "stream:1",
      sequence: 1,
      type: "product.surface.command_completed" as const,
      command: "readGoal",
      at: 10
    }
    expect(isSurfaceEnvelope({
      ok: true,
      command: "readGoal",
      value: { kind: "product.goal.found", goal: model },
      event
    }, "readGoal")).toBe(true)
    expect(isSurfaceEnvelope({
      ok: true,
