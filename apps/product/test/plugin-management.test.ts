...
describe("Product Plugin management boundary", () => {
  ...
  it("delegates one canonical state and isolates Product event listeners", async () => {
    const port = new FakePluginManagementPort()
    const shell = await createProduct({ pluginManagement: port })
    ...
    try {
      ...
      expect(events).toEqual([{
        kind: "product.plugin-management.invalidated",
        sequence: 1,
        at: 7_000,
...
async function createProduct(
  ...
  const storeDir = await mkdtemp(join(tmpdir(), "wanex-product-plugin-management-"))
