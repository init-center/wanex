...
describe("Product Team boundary", () => {
  ...
  it("lists, reads, selects, and invalidates through one safe port", async () => {
    ...
    const stateStore = createMemoryStateStore()
    const app = await createProduct({ stateStore, teamConversations: port })
    ...
    try {
      ...
      expect(selected.conversationId).toBe("team_product")
      ...
      await expect(app.teamConversations.readConversation()).resolves
        .toMatchObject({
          kind: "product.team-conversation.found",
          page: {
            conversation: { conversationId: "team_product" },
            messages: [
  ...
  it("fails closed for unsupported policy and non-public message parts", () => {
    ...
    expect(() => projectTeamConversationSummary({
      ...
      rounds: page.rounds
    })).toThrow("unsupported Product Team conversation mode: hybrid")
  ...
  it("carries Team commands and invalidations through the strict Surface", async () => {
    const port = new FakeTeamPort(teamPage())
    const app = await createProduct({ teamConversations: port })
    ...
    try {
      await expect(client.listTeamConversations({ limit: 500 })).resolves
        .toMatchObject({
          ...
          value: {
            kind: "product.team-conversation-list",
            conversations: [{ conversationId: "team_product" }]
          }
        })
      expect(port.lastListLimit).toBe(100)
      await expect(client.selectTeamConversation({
        ...
        ok: true,
        value: { conversationId: "team_product" }
      })
      await expect(client.readTeamConversation()).resolves.toMatchObject({
        ok: true,
        value: {
          kind: "product.team-conversation.found",
          page: { conversation: { conversationId: "team_product" } }
        }
      })
      await expect(client.submitTeamRound({
        ...
        ok: true,
        value: { kind: "product.team-round.submitted" }
      })
...
async function createProduct(
  ...
  const storeDir = await mkdtemp(join(tmpdir(), "wanex-product-team-"))
    ...
    messages: [
      {
        ...
        discussionRoundId: "round_product",
        kind: "message",
        state: "routed",
        targets: [{ kind: "all" }],
        content: [
