...
const SESSION_ID = "ses_eval_product_guided_follow_up"
...
export const guidedFollowUpScenario = createEvalScenario({
  id: "product.guided-follow-up-operational",
  title:
    "Product queues an exact next Turn without rebinding or interrupting the current Turn",
  tags: [
    "product",
    "conversation",
    "guided-follow-up",
  ...
  async run(context) {
    const storeDir = await mktemp("wanex-eval-product-guided-follow-up-")
    ...
    try {
      ...
      assert(
        ...
        "parent should return an active durable Product operation"
      )
      await withTimeout(
      ...
      assert(
        ...
        "forged Product operation identity should fail without replacing canonical parent state"
      )
      ...
      assert(
        childInput.intent === "follow_up" &&
          ...
          childInput.origin.sourceRef === "product.guided-follow-up" &&
      ...
      assert(
        staleOpaque.kind === "product.conversation-operation.rejected" &&
          staleOpaque.reason === "operation_identity_mismatch",
        "the promoted child should make the parent Product identity stale"
      )
      const staleDurable = await app.dispatchProductCommand({
      ...
      assert(
        ...
        "stale Product and durable references must not create extra work"
      )
      ...
      const transcript = terminal.conversation.historyRows.map((row) => ({
        role: row.role,
        text: productConversationRowText(row)
      }))
      assert(
        transcript.some(
