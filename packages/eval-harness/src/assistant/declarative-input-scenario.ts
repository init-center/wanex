...
import { createAssistantPluginHost } from "@wanex/assistant-plugin-host"
import { createTuiSurface, runTuiLineSession } from "@wanex/tui"
...
export const declarativeCommandInputAssistantScenario = createEvalScenario({
  id: "assistant.declarative-command-input",
  title:
    "Declarative command input spans Web, TUI, and durable plugin execution",
  tags: ["assistant", "command-input", "schema", "web", "tui", "plugin"],
  async run(context) {
    ...
    const executionEnvironment = new NativeExecutionEnvironment({
      environmentId: "native_eval_declarative_plugin",
    ...
    try {
      ...
      const assistantClient = assistant.client

      const catalog = await assistantClient.readAssistantCommands()
      ...
      assert(
        workbenchMode.ok && workbenchMode.snapshot.view.mode === "workbench",
      ...
      assert(
        reference !== undefined,
        "Web execution should expose a durable job"
      )
      ...
      const activity = await assistantClient.readExecutionReference(reference)
      assert(
        activity.ok &&
      ...
      const webEntry = entryByName(footprint, "@wanex/assistant-ui")
      const tuiEntry = entryByName(footprint, "@wanex/tui")
      const hostEntry = entryByName(footprint, "@wanex/assistant-plugin-host")
      assert(
