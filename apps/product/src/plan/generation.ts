...
export function createPlanGenerationCoordinator(request: {
  ...
  function emit(
    ...
    const event = {
      kind: "product.plan.invalidated" as const,
    ...
    try {
      const proposal = await request.backend.commands.generatePlanProposal({
        ...
        idempotencyKey:
          ...
          `product:plan-generation:${retained.model.operationId}`,
  ...
  return {
    ...
    read(input) {
      ...
      if (retained === undefined) {
        return { kind: "product.plan-generation.missing", operationId }
      }
      return {
        kind: "product.plan-generation.found",
    ...
    async dismiss(input) {
      return await serialize(async () => {
        ...
        return {
          kind: "product.plan-generation.dismissed",
          operationId: retained.model.operationId
        }
      })
    },
