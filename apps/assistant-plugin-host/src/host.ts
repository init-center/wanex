...
import type { CommandExecutionInvalidationListener } from "@wanex/assistant"
...
import { createPluginActionAssistantCommandExecutor } from "./plugin-action/index.js"
import type {
  CreateAssistantPluginHostOptions,
  AssistantPluginHost,
  AssistantPluginHostRefreshResult,
  AssistantPluginHostRefreshStatus,
  AssistantPluginHostRunOnceResult,
  AssistantPluginHostStatus,
  PluginActionHostFactory,
} from "./types.js"

export async function createAssistantPluginHost(
  options: CreateAssistantPluginHostOptions,
): Promise<AssistantPluginHost> {
  ...
  const registry = new PluginActionHostRegistry(
  ...
  const worker = createPluginActionWorker({
    ...
    ...(options.worker.permissionGuard === undefined
      ? {}
      : { permissionGuard: options.worker.permissionGuard }),
  ...
  const commandAssistantBinding = {
    extensions: { source: catalog.source },
    assistantCommands: {
      extensionExecutor: createPluginActionAssistantCommandExecutor({
        port: {
  ...
  let lastRefresh: AssistantPluginHostRefreshStatus | undefined
  let refreshPromise: Promise<AssistantPluginHostRefreshResult> | undefined
  let refreshRequested = false

  const status = (): AssistantPluginHostStatus => {
    ...
    if (result.status !== "idle" && result.job !== null) {
      for (const listener of executionInvalidationListeners) {
        try {
          ...
          // Assistant listeners cannot affect worker settlement.
        }
      }
    }
  }

  const refreshOnce = async (): Promise<AssistantPluginHostRefreshResult> => {
    ...
    try {
      ...
      const result: AssistantPluginHostRefreshResult = {
        status: "succeeded",
  ...
  const drainRefresh = async (): Promise<AssistantPluginHostRefreshResult> => {
    refreshRequested = false
    let result: AssistantPluginHostRefreshResult = await refreshOnce()
  ...
  const refresh = (): Promise<AssistantPluginHostRefreshResult> => {
...
const defaultPluginActionHostFactory: PluginActionHostFactory = (options) =>
  createTrustedSubprocessPluginActionHostFromInstall({
