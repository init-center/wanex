...
describe("Assistant command execution events", () => {
  it("invalidates one durable job reference without exposing execution details", async () => {
    ...
    const shell = await createShell({
      ...
      assistantCommands: { executionInvalidations: source },
    })
    const surface = createSurfaceAdapter(shell, {
      now: () => 51_000,
      streamId: "assistant-command-execution-events",
    })
    ...
    try {
      for (const listener of listeners) {
        listener({ kind: "job", id: "job_assistant_execution_event" })
      }
      ...
      expect(observed).toEqual([
        expect.objectContaining({
          ...
          commandExecution: {
            ...
            reference: {
              kind: "job",
              id: "job_assistant_execution_event",
            },
          },
        }),
      ])
      ...
      expect(canonical).toMatchObject({
        ...
        value: {
          kind: "missing",
          reference: { kind: "job", id: "job_assistant_execution_event" },
        },
      })
    } finally {
      unsubscribe()
