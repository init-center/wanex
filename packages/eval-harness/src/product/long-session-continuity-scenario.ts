...
export const longSessionContinuityScenario = createEvalScenario({
  id: "product.long-session-continuity-operational",
  title: "Started Product Host compacts long context and exposes capacity recovery",
  tags: [
    "product",
    "context",
    "memory",
  ...
  async run(context) {
    const fixture = await createConversationSettlementFixture({
      ...
      prefix: "wanex-eval-product-long-session-"
    })
    ...
    try {
      ...
      assert(first.ok, "first bounded Product Turn should be admitted")
      assert(
        (await firstSettlement).turn.state === "succeeded",
        "first bounded Product Turn should succeed"
      )
      ...
      assert(
        ...
        "first Product Turn should persist its exact canonical input and response"
      )
      ...
      assert(second.ok, "boundary-crossing Product Turn should be admitted")
      const secondReceipt = await secondSettlement
      assert(
        secondReceipt.turn.state === "succeeded",
        "boundary-crossing Product Turn should complete after inline compaction"
      )
      ...
      assert(
        transcript.kind === "product.session-transcript.found" &&
          transcript.transcript.rows.some(
            (row) => productConversationRowText(row) === oldInput
          ) &&
          transcript.transcript.rows.some(
            (row) => productConversationRowText(row) === currentInput
          ),
        "Product transcript should keep both canonical user messages visible"
      )
      ...
      assert(failedSubmission.ok, "oversized Product Turn should remain durably admitted")
      ...
      assert(
        failedOperation?.error?.code ===
          ...
          failedOperation.transcript.rows.some(
            (row) =>
              row.role === "user" &&
              productConversationRowText(row) === oversizedInput
          ),
        "Product should expose bounded capacity evidence and the canonical input"
      )
      ...
      assert(
        selected.ok &&
          selected.snapshot.view.settings.profile.activeModelEndpointId ===
            LARGE_ENDPOINT_ID,
        "Product should explicitly select the larger configured endpoint"
      )
