...
export interface StartLocalWebAppOptions {
  ...
  /**
   * Trusted-host credential persistence. It is never exposed through Product
  ...
  readonly web?: LocalWebHostOptions;
}

export interface StartLocalProductHostOptions {
  readonly storage: LocalStorageConfig;
  readonly serviceBin?: string;
...
export interface LocalProductHost {
...
export interface LocalWebApp {
  ...
  readonly url: string;
  readSnapshot(): Promise<LocalSnapshot>;
  close(): Promise<void>;
}

export interface LocalModelCatalogRefreshSuccess {
  readonly kind: "local-host.model-catalog.refreshed";
...
export interface LocalModelCatalogRefreshFailure {
  readonly kind: "local-host.model-catalog.refresh-failed";
  readonly code:
    | "timeout"
    | "transport_failed"
    | "unexpected_status"
...
export interface LocalConversationModelSuggestions {
  readonly kind: "local-host.conversation-model-suggestions";
...
export interface LocalSnapshot {
  readonly kind: "local-host.snapshot";
  readonly url: string;
  readonly settings: SettingsReadModel;
  readonly modelEndpoints: ModelEndpointListReadModel;
  readonly web: Snapshot;
  readonly privacy: LocalSnapshotPrivacy;
}

export interface LocalSnapshotPrivacy {
...
export interface LocalConfigureImageGenerationCapabilityResult {
  readonly kind: "local-host.image-generation-capability.configured";
...
export interface LocalRemoveProviderResult {
  readonly kind: "local-host.provider.removed";
  readonly connectionId: string;
