...
Shared browser Assistant UI capability for `@wanex/assistant`.

This package is not a lifecycle host or product executable. It consumes the application
surface client and projects canonical Assistant state into a typed `Snapshot`.
It does not open storage, resolve the native System
...
const client = createHttpClient({
  requestPath: "/wanex/assistant/request",
  eventStreamPath: "/wanex/assistant/events",
  hostSessionToken,
});
...
Any trusted host can serve this renderer at `/` with one same-origin
...
current snapshot. No response contains rendered Assistant HTML.
...
The Assistant Surface remains the sole runtime authority for each complete typed
...
JSON string field, or compatibility decoder between the renderer and Assistant.

## Rendering Contract

The renderer owns ephemeral UI state such as draft text, panel visibility,
upload progress, object-URL previews, and stream connection status. Assistant
...
The primary Assistant layout is chat-first:
...
```sh
pnpm --filter @wanex/assistant-ui build:browser
```

The package check verifies that the generated bundle is current. The browser
bundle contains only React, Assistant Web, and their explicitly allowed browser
...
When the event stream closes, the Assistant remains usable and the top bar exposes
