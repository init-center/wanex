...
describe("Assistant Host secret-store composition", () => {
  it("routes owned credentials to the injected store and external refs to the fallback resolver", async () => {
    ...
    const secrets = await composeLocalSecretStore({
      storage: {
        kind: "store-dir",
        storeDir: "/tmp/wanex-assistant-host-secret-store-test"
      },
      credentialStore,
  ...
  it("fails closed when no resolver owns a non-local secret scheme", async () => {
    ...
    const secrets = await composeLocalSecretStore({
      storage: {
        kind: "store-dir",
        storeDir: "/tmp/wanex-assistant-host-secret-store-no-fallback"
      },
      credentialStore
    })
  ...
  it("derives stable isolated namespaces from resolved local stores", () => {
    const first = localSecretNamespace({
      kind: "store-dir",
      storeDir: "/tmp/wanex-assistant-host-secret-store-a"
    })
    const second = localSecretNamespace({
      kind: "store-dir",
      storeDir: "/tmp/wanex-assistant-host-secret-store-b"
    })
