...
describe("@wanex/local-host Web host", () => {
  it("serves the sole browser shell and typed request envelopes over Node HTTP", async () => {
    await withNodeHost(async ({ controller, host }) => {
      ...
      expect(shell.headers.get("set-cookie")).toMatch(
        /^wanex_host_session_[a-f0-9]{16}=[A-Za-z0-9_-]{43}; HttpOnly; SameSite=Strict; Path=\/wanex\/web$/
      )
      ...
      expect(html).toContain('data-request-path="/wanex/web/request"')
      expect(html).toMatch(/data-host-session-token="[A-Za-z0-9_-]{43}"/)
      expect(html).toContain(
        'data-event-stream-path="/wanex/web/events"'
      )
      expect(html).toContain(
        'data-resource-delivery-prepare-path="/wanex/web/resource-delivery/prepare"'
      )
      ...
      const submitted = await postJson(`${host.url}/wanex/web/request`, {
        kind: "web.request",
        operation: "dispatchAction",
      ...
      const invalid = await postJson(`${host.url}/wanex/web/request`, {
        kind: "web.request",
  ...
  it("exposes only an authenticated parameterless model catalog refresh", async () => {
    let refreshes = 0
    let suggestions = {
      kind: "local-host.conversation-model-suggestions" as const,
      providers: {
    ...
    await withNodeHost(async ({ host }) => {
      ...
      const path = `${host.url}/wanex/web/model-catalog-refresh`
  ...
  it("authenticates the event stream and rejects malformed cursors", async () => {
    await withNodeHost(async ({ host }) => {
      const eventUrl = `${host.url}/wanex/web/events`
  ...
  it("force-closes active loopback requests during Node host shutdown", async () => {
    ...
    if (address === null || typeof address === "string") {
      throw new Error("test Product host did not expose a TCP address")
    }
    ...
    await accepted
    socket.write([
      "POST /wanex/web/request HTTP/1.1",
      "Host: 127.0.0.1",
      "Content-Type: application/json",
      "Content-Length: 100",
  ...
  it("keeps binary attachment upload separate from the JSON command port", async () => {
    ...
    const attachments: LocalAttachmentUploadPort = {
      async uploadAttachment(request) {
        ...
        return {
          kind: "local-host.attachment-uploaded",
          attachment: {
  ...
  it("protects and bounds trusted capability setup requests", async () => {
    ...
    await withNodeHost(
      async ({ host }) => {
        const path = "/wanex/web/capability-setup"
        const valid = {
  ...
  it("prepares opaque resource delivery and serves GET, HEAD, and one byte Range", async () => {
    ...
    await withNodeHost(async ({ host }) => {
      const preparePath = "/wanex/web/resource-delivery/prepare"
      const prepareBody = {
      ...
      expect(preparedBody.delivery.url).toMatch(
        /^\/wanex\/web\/resource-delivery\?token=wrd_[A-Za-z0-9_-]{43}$/
      )
...
function successfulCapabilitySetup(request: {
  ...
  return {
    kind: "local-host.capability-setup.continued",
    setup: {
      kind: "local-host.image-generation-capability.configured",
      endpoint,
      readiness: {
        requirement: {
          operation: "image.generate",
...
async function readHostSessionCookie(url: string): Promise<string> {
  ...
  if (cookie === undefined || !/^wanex_host_session_[a-f0-9]{16}=/.test(cookie)) {
    throw new Error("product Host shell did not set its session cookie")
  }
  return cookie
}
