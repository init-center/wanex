...
export interface LocalCliEnvironment {
  ...
  readonly WANEX_LOCAL_HOST_OPEN?: string
  readonly WANEX_LOCAL_HOST_SMOKE?: string
  readonly WANEX_LOCAL_HOST_SETUP_PROVIDER?: string
  readonly WANEX_LOCAL_HOST_SUMMARY_FORMAT?: string
  readonly WANEX_LOCAL_HOST_HOSTNAME?: string
  readonly WANEX_LOCAL_HOST_PORT?: string
  readonly WANEX_LOCAL_HOST_STORE_DIR?: string
  readonly WANEX_LOCAL_HOST_PROFILE_ROOT?: string
  readonly WANEX_LOCAL_HOST_PROFILE_ID?: string
  readonly WANEX_LOCAL_HOST_SERVICE_BIN?: string
  readonly WANEX_LOCAL_HOST_MODEL_ENDPOINT_ID?: string
  readonly WANEX_LOCAL_HOST_PROVIDER_CONNECTION_ID?: string
  readonly WANEX_LOCAL_HOST_PROVIDER_PROTOCOL?: string
  readonly WANEX_LOCAL_HOST_PROVIDER_ID?: string
  readonly WANEX_LOCAL_HOST_PROVIDER_MODEL_ID?: string
  readonly WANEX_LOCAL_HOST_MODEL_OPERATIONS?: string
  readonly WANEX_LOCAL_HOST_MODEL_INPUT_MODALITIES?: string
  readonly WANEX_LOCAL_HOST_MODEL_OUTPUT_MODALITIES?: string
  readonly WANEX_LOCAL_HOST_MODEL_FEATURES?: string
  readonly WANEX_LOCAL_HOST_MODEL_REASONING_REPLAY?: string
  readonly WANEX_LOCAL_HOST_PROVIDER_BASE_URL?: string
  readonly WANEX_LOCAL_HOST_PROVIDER_SECRET_REF?: string
  readonly WANEX_LOCAL_HOST_MODEL_ENDPOINTS_FILE?: string
  readonly WANEX_LOCAL_HOST_MODEL_ENDPOINTS_JSON?: string
  readonly WANEX_LOCAL_HOST_ACTIVE_MODEL_ENDPOINT_ID?: string
...
export function parseLocalCliOptions(
  ...
  const open =
    ...
    parseLocalCliBoolean(
      env.WANEX_LOCAL_HOST_OPEN,
      "WANEX_LOCAL_HOST_OPEN"
    )
  const smoke =
    flags.has("smoke") ||
    parseLocalCliBoolean(
      env.WANEX_LOCAL_HOST_SMOKE,
      "WANEX_LOCAL_HOST_SMOKE"
    )
  const setupProvider =
    flags.has("setup-provider") ||
    parseLocalCliBoolean(
      env.WANEX_LOCAL_HOST_SETUP_PROVIDER,
  ...
  const summaryFormat = parseLocalCliSummaryFormat(
    flags.get("summary-format") ??
      env.WANEX_LOCAL_HOST_SUMMARY_FORMAT ??
      "text"
  )
  const hostname =
    flags.get("hostname") ?? env.WANEX_LOCAL_HOST_HOSTNAME ?? "127.0.0.1"
  const port = optionalPort(
    flags.get("port") ?? env.WANEX_LOCAL_HOST_PORT
  )
  const serviceBinInput =
    flags.get("service-bin") ??
    env.WANEX_LOCAL_HOST_SERVICE_BIN ??
  ...
  const storage = parseStorage({
    ...
    storeDir:
      flags.get("store-dir") ??
      env.WANEX_LOCAL_HOST_STORE_DIR ??
      env.WANEX_STORE_DIR,
    profileRoot:
      flags.get("profile-root") ?? env.WANEX_LOCAL_HOST_PROFILE_ROOT,
    profileId:
      flags.get("profile-id") ?? env.WANEX_LOCAL_HOST_PROFILE_ID
  })
...
function parseStorage(input: {
  ...
  return {
    ...
    rootDir: resolvePath(
      input.cwd,
      input.profileRoot ?? ".wanex-local-host"
    ),
