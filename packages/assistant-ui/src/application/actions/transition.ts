...
export function nextConversationAfterAction(request: {
  ...
  if (
    ...
    request.actionResult.value.kind === "assistant.plan-execution.submitted" &&
...
function isPlanSourceResult(value: unknown): value is PlanSourceResult {
  return (
    ...
    (value.kind === "assistant.plan-generation" ||
      ...
      value.kind === "assistant.plan-generation.missing" ||
      value.kind === "assistant.plan-proposal.found" ||
      value.kind === "assistant.plan-proposal.missing" ||
      value.kind === "assistant.plan-proposal.no-selection" ||
