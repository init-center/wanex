...
const reference = {
  sessionId: "ses_assistant_projection",
  inputId: "input_assistant_projection",
  turnId: "turn_assistant_projection",
  jobId: "job_assistant_projection",
} as const;

describe("Assistant conversation operation projection", () => {
  it("preserves ordered visible Parts while correlating and redacting Tool activity", () => {
    ...
    expect(tool).toHaveProperty("key", expect.stringMatching(/^assistant_conversation_part_/));
  ...
  ] as const)("maps durable Tool state %s to Assistant state %s", (
    executionState,
    expectedState,
  ) => {
    const source = foundOperation({
  ...
  it("projects pending approvals with opaque identity and bounded presentation", () => {
    ...
    expect(item?.approvalId).toMatch(/^assistant_conversation_approval_/);
    const serialized = JSON.stringify(projected);
