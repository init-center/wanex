...
import {
  createPluginActionProductCommandExecutor,
...
describe("@wanex/plugin-command-host", () => {
  ...
  it("adapts product command execution to a bounded durable job submission", async () => {
    const port = recordingPort()
    const executor = createPluginActionProductCommandExecutor({
      port,
