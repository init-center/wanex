...
import {
  startAssistantDemoHost,
  type AssistantDemoHost
...
describe("Assistant Host demo", () => {
  it("starts from the source entry with the default workspace artifact", async () => {
    ...
    expect(output.stdout).toContain("Wanex Assistant Host demo running")
  ...
  it("falls back to defaults and environment values", () => {
    ...
    const options = parseLocalDemoOptions(rootDir, [], {
      WANEX_ASSISTANT_WEB_PORT: "58000",
      WANEX_ASSISTANT_WEB_SESSION_ID: "ses_env",
      WANEX_ASSISTANT_WEB_SEED_TEXT: "env turn",
  ...
  it("can disable demo seeding from the environment", () => {
    const rootDir = "/repo"
    const options = parseLocalDemoOptions(rootDir, [], {
      WANEX_ASSISTANT_WEB_NO_SEED: "true"
    })
  ...
  it("cleans up the default temporary demo store on close", async () => {
    const demo = await startTestAssistantDemoHost({
      hostname: "127.0.0.1",
      serviceBin,
  ...
  it("starts the blank demo host and accepts the first conversation over HTTP", async () => {
    ...
    const response = await postJson(`${demo.url}/wanex/assistant/request`, {
      kind: "web.request",
  ...
  it("keeps chat focused and exposes workbench and diagnostics explicitly", async () => {
    ...
    const workbench = await postJson(
      `${demo.url}/wanex/assistant/request`,
      {
        kind: "web.request",
        operation: "dispatchAction",
  ...
  it("persists assistant renderer state across demo host restarts", async () => {
    const root = await createTestTempDir("wanex-web-demo-state-")
    const storeDir = join(root, "store")

    const first = await startTestAssistantDemoHost({
      hostname: "127.0.0.1",
      storeDir,
      serviceBin,
    ...
    const changed = await postJson(`${first.url}/wanex/assistant/request`, {
      kind: "web.request",
      operation: "dispatchAction",
    ...
    const preferences = await postJson(
      `${first.url}/wanex/assistant/request`,
      {
        kind: "web.request",
        operation: "dispatchAction",
    ...
    const mode = await postJson(`${first.url}/wanex/assistant/request`, {
      kind: "web.request",
      operation: "dispatchAction",
    ...
    const second = await startTestAssistantDemoHost({
      hostname: "127.0.0.1",
      storeDir,
      serviceBin,
      sessionId: "ses_demo_persisted_state",
...
async function startTestAssistantDemoHost(
  options: Parameters<typeof startAssistantDemoHost>[0]
): Promise<AssistantDemoHost> {
  const demo = await startAssistantDemoHost(options)
...
async function requestSnapshot(url: string): Promise<any> {
  const response = await postJson(`${url}/wanex/assistant/request`, {
    kind: "web.request",
    operation: "snapshot",
