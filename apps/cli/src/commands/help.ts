export function helpValue(): unknown {
  return {
    ...
    usage: [
      ...
      "wanex provider set <id> --kind <fake|openai-compatible|anthropic|deepseek> --provider-id <id> --model <model> [--input-modalities <csv>] [--output-modalities <csv>] [--base-url <url>] [--secret-ref <ref>]",
      "wanex provider get <id>",
      "wanex side-query <text> [--session <id>] [--provider <id>] [--max-output-tokens <n>] [--timeout-ms <n>]",
      "wanex run <text> [--session <id>] [--provider <id>] [--max-steps <n>] [--instructions-cwd <dir> [--instructions-project-root <dir>] [--instructions-global-dir <dir>] [--trust-project-instructions]] [--skills-cwd <dir> [--skills-project-root <dir>] [--skills-global-dir <dir[,dir]>] [--trust-project-skills] [--activate-skill-tool]] [--store <dir> | --store-profile <id> [--store-root <dir>]] [--service-bin <path>]"
    ]
  }
}
