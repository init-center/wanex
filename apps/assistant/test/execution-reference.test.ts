...
describe("application execution activity surface", () => {
  it("resolves bounded job activity through shell, surface, and client", async () => {
    const storeDir = await mkdtemp(join(tmpdir(), "wanex-assistant-ref-"))
    ...
    const app = await createShell({
      ...
      modelEndpoint: assistantTestModelEndpoint({
        endpointId: "assistant.execution-reference",
        modelId: "assistant.execution-reference-model"
      })
    })
    ...
    try {
      await app.dispatchAssistantCommand({
        ...
        input: {
          ...
          sessionId: "ses_assistant_app_reference",
          jobId: "job_assistant_app_reference"
        }
      })

      await waitForJob(app, "job_assistant_app_reference")

      await expect(
        app.readExecutionReference({
          kind: "job",
          id: "job_assistant_app_reference"
        })
      ...
      const found = await client.readExecutionReference(
        { kind: "job", id: "job_assistant_app_reference" },
      ...
      expect(found).toMatchObject({
        ...
        value: {
          kind: "found",
          reference: { kind: "job", id: "job_assistant_app_reference" },
          activity: {
