...
export interface LocalCliSmokeResult {
  readonly kind: "local-host.cli.smoke-result"
  readonly ok: boolean
...
export async function runLocalCliSmoke(
  ...
  const layout = await postJson(`${input.app.url}/wanex/web/request`, {
    kind: "web.request",
  ...
  const conversation = await postJson(`${input.app.url}/wanex/web/request`, {
    kind: "web.request",
    operation: "dispatchAction",
    ...
    action: {
      ...
      input: {
        text: "hello from local host smoke"
      }
    }
  }, hostSessionToken)
    ...
    conversationAction: check(
      readPath(conversation, [
        ...
        "kind"
          ...
          "product.conversation-operation" &&
        readPath(conversation, [
          "actionResult",
          "snapshot",
          "view",
          ...
          "hello from local host smoke" &&
        snapshot.web.conversation.operation?.kind ===
          "product.conversation-operation",
      "conversation action submits through the Web request envelope"
    ),
    privacy: check(
      !containsSensitiveText(productValues, input.options.serviceBin) &&
        !containsStoragePath(productValues, input.options),
      "smoke output does not leak host-only paths through product values"
    )
  }
  return {
...
async function waitForConversationTerminal(
  ...
  throw new Error("local host smoke conversation did not finish")
}
...
function readHostSessionToken(html: string): string {
  ...
  if (match?.[1] === undefined) {
    throw new Error("Product browser shell did not include a session token")
  }
  return match[1]
}
