...
const PLAN_ACTOR_ID = "product-user"
...
export async function readPlanProposal(request: {
  ...
  if (proposalId === undefined) {
    return { kind: "product.plan-proposal.no-selection" }
  }
  const view = await request.backend.commands.readPlanProposal({ proposalId })
  if (view === null) {
    return { kind: "product.plan-proposal.missing", proposalId }
  }
  return {
...
export async function listPlanProposals(request: {
  ...
  return {
    kind: "product.plan-proposal-list",
    sessionId,
    proposals: views
...
function idempotencyKey(
  ...
  return supplied === undefined
    ? `product:plan:${operation}:${proposalId}:${randomUUID()}`
