...
import type { SurfaceClient, SurfaceEvent } from "@wanex/product/surface"
...
export const extensionPluginActionProductPathScenario = createEvalScenario({
  ...
  title: "Plugin commands hot-update through the canonical Product event path",
  ...
  async run(context) {
    ...
    const product = await createEvalPluginCommandProduct({
      ...
      modelEndpoint: evalFakeModelEndpoint(
        "eval-extension-product",
        "eval-extension-product-model"
      )
    })
    const events: SurfaceEvent[] = []
    const eventCount = () => events.length
    const unsubscribe = product.client.subscribeSurfaceEvents((event) => {
    ...
    try {
      const initialCatalog = await product.client.readProductCommands()
      assert(initialCatalog.ok, "initial Product command catalog should load")
      assert(
        !initialCatalog.value.commands.some((command) => command.id === COMMAND_ID),
        "zero-Plugin startup should not expose the Plugin command"
      )
      assert(eventCount() === 0, "zero-Plugin startup should not emit invalidation")
      ...
      const v1Catalog = await requireCommandVersion(product.client, "1.0.0")
      assert(
        events[0]?.commandCatalog?.revision === v1Catalog.revision,
        "v1 invalidation should identify the canonical Product generation"
      )
      ...
      assert(eventCount() === 2, "version replacement should emit one invalidation")
      const v2Catalog = await requireCommandVersion(product.client, "2.0.0")
      assert(
        events[1]?.commandCatalog?.revision === v2Catalog.revision,
        "v2 invalidation should identify the canonical Product generation"
      )
      ...
      const disabledCatalog = await product.client.readProductCommands()
      assert(disabledCatalog.ok, "disabled Product command catalog should load")
      assert(
      ...
      assert(
        !disabledCatalog.value.commands.some((command) => command.id === COMMAND_ID),
        "disabled Plugin command should disappear from the canonical catalog"
      )
      const rejected = await product.client.executeProductCommand({
      ...
      assert(eventCount() === 3, "identical refresh should not emit invalidation")
      ...
      return {
        ...
        invalidationRevisions: events.map((event) =>
          event.commandCatalog?.revision ?? null
        ),
...
async function requireCommandVersion(
  ...
  assert(catalog.ok, `Product command catalog should load for ${expectedVersion}`)
  const command = catalog.value.commands.find((item) => item.id === COMMAND_ID)
  assert(command !== undefined, `Plugin command should resolve for ${expectedVersion}`)
  const handler = parsePluginActionHandlerRef(command.handlerRef)
  assert(handler !== undefined, "Plugin command should preserve its typed handler ref")
  assert(
...
async function executeAndComplete(
  client: SurfaceClient,
  commandHost: Awaited<ReturnType<typeof createPluginCommandHost>>,
  text: string
): Promise<string> {
  ...
  assert(job !== undefined, "Plugin action submission should expose a job reference")
  ...
  assert(
    ...
    "Product should resolve the completed Plugin activity from the shared store"
  )
  return job.id
}

function assertPrivateInvalidationEvents(events: readonly SurfaceEvent[]): void {
  ...
  assert(
    events.every(
      (event) =>
        event.command === "readProductCommands" &&
        event.commandCatalog?.kind === "product.command-catalog.invalidated" &&
