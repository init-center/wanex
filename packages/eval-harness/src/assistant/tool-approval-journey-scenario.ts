...
const SECRET_REF = "static://eval-assistant-tool-approval";
const SECRET_VALUE = "eval-assistant-tool-approval-secret";
const RAW_TOOL_INPUT = "eval-assistant-approval-raw-input";
const TOOL_NAME = "assistant_external_publish";
const APPROVE_SESSION_ID = "ses_eval_assistant_approval_approve";
const DENY_SESSION_ID = "ses_eval_assistant_approval_deny";
const CANCEL_SESSION_ID = "ses_eval_assistant_approval_cancel";

export const toolApprovalJourneyScenario = createEvalScenario({
  id: "assistant.tool-approval-journey",
  title: "Assistant Web and TUI resolve bounded Tool approvals through App authority",
  tags: ["assistant", "web", "tui", "tool", "approval", "assistant-path"],
  async run(context) {
    const storeDir = join(context.storeDir, "assistant-tool-approval-journey");
    ...
    globalThis.fetch = (async () => {
      ...
      return providerCalls % 2 === 1
        ? openAIToolCallResponse(`call_assistant_approval_${providerCalls}`)
    ...
    tools.register({
      ...
      runtimeBinding: createToolRuntimeBinding({
        implementationId: "wanex.eval.assistant.tool-approval",
      ...
      async invoke(invocation) {
        ...
        assert(
          ...
          "approved Assistant Tool must receive exact trusted input",
        );
        return {
          outcome: "succeeded",
    ...
    const app = await createShell({
      ...
      modelEndpoint: evalOpenAICompatibleModelEndpoint({
        id: "eval-assistant-approval-provider",
        modelId: "eval-assistant-approval-model",
        baseUrl: "https://assistant-approval.example.test/v1",
    ...
    try {
      ...
      assert(submitted.ok, "Assistant approval fixture must be admitted");
      const approvalSnapshot = await waitForApproval(web);
      const approval = approvalSnapshot.conversation.operation?.approvals?.items[0];
      assert(approval !== undefined, "Assistant must expose one pending approval");
      assert(
      ...
      assert(
        ...
        "Assistant approval identity must be opaque",
      );
      for (const forbidden of [
        ...
        SECRET_REF,
        SECRET_VALUE,
        "assistant-approval:private-authorization",
        "inputSchema",
        "leaseToken",
      ]) {
        assert(
          ...
          `Assistant approval projection leaked ${forbidden}`,
        );
      }
      assert(
        ...
        "Web snapshot and TUI must project the same bounded Assistant approval",
      );
      ...
      const stale = await app.resolveTrackedConversationApproval({
        ...
        reason: "stale Assistant decision probe",
      });
      assert(
        stale.kind === "assistant.conversation-operation.rejected" &&
          stale.reason === "approval_revision_stale",
        "stale Assistant approval revision must fail closed",
      );
      const unknown = await app.resolveTrackedConversationApproval({
        sessionId: APPROVE_SESSION_ID,
        approvalId: "assistant_conversation_approval_unknown",
        expectedApprovalRevision: approval.approvalRevision,
        decision: "approve_once",
        reason: "unknown Assistant approval probe",
      });
      assert(
        unknown.kind === "assistant.conversation-operation.rejected" &&
          unknown.reason === "approval_not_found",
        "unknown Assistant approval identity must fail closed",
      );
      ...
      assert(
        ...
        "approved Assistant Turn must invoke the Tool exactly once and succeed",
      );
      ...
      assert(denySubmitted.ok, "Assistant denial fixture must be admitted");
      ...
      assert(
        ...
        "TUI must resolve denial through the shared Assistant Surface command",
      );
      const denyTerminal = await waitForTerminal(web);
      assert(
        denyTerminal.conversation.state === "succeeded" && toolCalls === 1,
        "denied Assistant Tool must invoke zero effects and continue canonically",
      );
      ...
      assert(cancelSubmitted.ok, "Assistant cancellation fixture must be admitted");
      ...
      assert(cancelled.ok, "Assistant cancellation must be accepted");
      ...
      const late = await app.resolveTrackedConversationApproval({
        ...
        decision: "approve_once",
        reason: "late Assistant approval probe",
      });
      assert(
        late.kind === "assistant.conversation-operation.rejected" &&
          late.reason === "approval_not_found",
        "cancelled approval must reject a late Assistant decision",
      );
      ...
      try {
        ...
        assert(
          ...
          "approved Assistant execution must have one physical attempt",
        );
        assert(
          deniedExecutions[0]?.state === "denied" &&
            deniedExecutions[0].attemptCount === 0 &&
            denialResults.length === 1,
          "denied Assistant execution must have zero attempts and one canonical result",
        );
        assert(
          cancelledExecutions[0]?.state === "cancelled" &&
            cancelledExecutions[0].attemptCount === 0,
          "cancelled Assistant execution must have zero attempts",
        );
        assert(
          providerCalls === 5 && toolCalls === 1,
          "Assistant approval journey must preserve exact Provider and Tool calls",
        );
        return {
...
class rovalRequiredPolicy implements ToolPermissionPolicy {
  snapshot() {
    return createToolRuntimeBinding({
      implementationId: "wanex.eval.assistant.tool-approval-policy",
      implementationRevision: "1",
    });
  }
...
async function waitForApproval(
  ...
  return await eventually(async () => {
    ...
    assert(
      ...
      "Assistant conversation has not reached approval wait",
    );
    return snapshot;
  });
}
