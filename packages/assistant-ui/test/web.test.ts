...
import { type SurfaceTransportRequest } from "@wanex/assistant/surface";
import {
...
describe("@wanex/assistant-ui", () => {
  it("rereads the canonical command catalog only after invalidation", async () => {
    ...
    await withWebSurface(
      async ({ assistantSurface, surface, observed }) => {
        const readCount = () => observed.filter(
          (request) =>
            request.operation === "dispatchSurfaceCommand" &&
            request.command.command === "readAssistantCommands",
        ).length;
        ...
        expect(
          assistantSurface.readSurfaceEvents().events.filter(
            (event) =>
  ...
  it("rereads the canonical schedule list only after schedule invalidation", async () => {
    ...
    await withWebSurface(
      async ({ assistantSurface, surface, observed }) => {
        ...
        await surface.reconcileEvents();
        expect(readCount()).toBe(2);
        expect(assistantSurface.readSurfaceEvents().events.some(
  ...
  it("tracks command job references and refreshes them during reconciliation", async () => {
    ...
    await withWebSurface(async ({ app, client }) => {
      ...
      const trackedClient = {
        ...client,
        async executeAssistantCommand() {
          const original = await client.executeAssistantCommand({
      ...
      const executed = await tracked.dispatchAction({
        ...
        input: { commandId: "assistant.status" },
      });
  ...
  it("projects assistant state into a safe typed web snapshot", async () => {
  ...
  it("dispatches actions and reconciles Surface events by cursor", async () => {
    await withWebSurface(async ({ app, assistantSurface, surface, observed }) => {
      await seedSession(
        app,
        "ses_assistant_app_web",
      ...
      const selected = await surface.dispatchAction({
        type: "select-session",
        sessionId: "ses_assistant_app_web",
      });
      ...
      const reconciled = await surface.dispatchAction({
        type: "open-workbench",
        input: { sessionId: "ses_assistant_app_web" },
      });
  ...
  it("renames, archives, and restores sessions through revision-fenced actions", async () => {
    await withWebSurface(async ({ app, surface }) => {
      const sessionId = "ses_assistant_app_web_lifecycle";
      await seedSession(app, sessionId, "Original web chat");
      await seedSession(
        app,
        "ses_assistant_app_web_lifecycle_second",
        "Second web chat",
      );
  ...
  it("loads earlier conversation history through the action boundary and preserves it on refresh", async () => {
    await withWebSurface(async ({ app, client }) => {
      ...
      expect(second.kind).toBe("assistant.conversation-operation.found");
  ...
  it("projects and reconciles the Goal Mode journey by invalidation", async () => {
    await withWebSurface(async ({ app, client }) => {
      ...
      await waitForAssistantGoalTerminal(app, started.goalId);
  ...
  it("falls back to command text input when the assistant catalog is unavailable", async () => {
    await withWebSurface(async ({ surface }) => {
      const snapshot = surface.snapshot();
      ...
      const unavailable = {
        ...
        commandCatalog: {
          ok: false as const,
          command: "readAssistantCommands" as const,
          error: {
...
async function withWebSurface(
  run: (context: {
    readonly app: Shell;
    readonly assistantSurface: ReturnType<typeof createSurfaceAdapter>;
  ...
  shellOptions: Pick<ShellOptions, "extensions" | "assistantCommands" | "schedules"> = {},
): Promise<void> {
  ...
  try {
    ...
    await run({ app, assistantSurface, client, surface, observed });
  } finally {
    await app.dispose();
  }
}
...
type WebExecutionInvalidationSource = NonNullable<
  NonNullable<ShellOptions["assistantCommands"]>["executionInvalidations"]
>;
...
async function waitForAssistantGoalTerminal(
  app: Shell,
  goalId: string,
): Promise<void> {
  for (let attempt = 0; attempt < 200; attempt += 1) {
