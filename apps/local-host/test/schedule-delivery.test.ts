...
} from "../src/application/product.js"
...
const hosts: StartedLocalProductHost[] = []

afterEach(async () => {
  while (hosts.length > 0) {
    const host = hosts.pop()
    if (host !== undefined) await closeStartedLocalProductHost(host)
  }
...
describe("Local Schedule durable delivery", () => {
  it("settles deterministic App admission and replays it idempotently", async () => {
    ...
    const submit = async (request: Parameters<
      StartedLocalProductHost["shell"]["trustedExecution"]["submitScheduledTick"]
    >[0]) => {
      calls.push(request)
      return {
  ...
  it("recovers an App admission committed before occurrence settlement", async () => {
    ...
    const host = await startLocalProductHostInternal({
  ...
  return {
    ...
    model: {
      ...
      catalog: {
        ...
        catalogId: `local-host.schedule.${id}`,
        revision: "1",
      },
    },
  }
