...
export function projectConversationFromResult(
  ...
  if (result.kind === "product.conversation-operation.cancel") {
    return projectConversationFromResult(result.operation, previous)
  }
  if (result.kind === "product.conversation-recovery.resolved") {
    return projectConversationFromResult(result.operation, previous)
  }
  if (result.kind === "product.conversation-approval.resolved") {
    return projectConversationFromResult(result.operation, previous)
  }
  ...
  if (result.kind === "product.conversation-operation.untracked") {
    if (
...
export function applyConversationHistory(
  ...
  if (result.kind === "product.session-transcript.no-session") {
...
export function prependConversationHistory(
  ...
  if (result.kind === "product.session-transcript.no-session") return conversation
  if (
    conversation.sessionId !== undefined &&
    conversation.sessionId !== result.sessionId
  ) return conversation
