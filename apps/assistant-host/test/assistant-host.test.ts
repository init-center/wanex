...
const assistantHosts: AssistantHost[] = []
const rawTeamStores: StorageTestStore[] = []

afterEach(async () => {
  while (assistantHosts.length > 0) {
    await assistantHosts.pop()?.close()
  }
  while (apps.length > 0) {
...
describe("@wanex/assistant-host", () => {
  it("owns a presentation-neutral Assistant and Team lifecycle", async () => {
    const host = await startAssistantHost({
      storage: {
        kind: "store-dir",
        storeDir: await tempDir("wanex-local-assistant-host-")
      },
      serviceBin,
      modelEndpoint: fakeEndpoint("local-assistant-host", "local-assistant-model")
    })
    ...
    await expect(host.shell.teamConversations.createConversation({
      ...
      idempotencyKey: "local-assistant-host-team"
  ...
  it("orders optional Plugin composition around the shared Assistant lifecycle", async () => {
    const events: string[] = []
  ...
  it("starts the local assistant web stack through the trusted host boundary", async () => {
    ...
    const app = await startAssistantWebApp({
      ...
      serviceBin,
      modelEndpoints: endpointCatalog(
        fakeEndpoint("assistant-host-test", "assistant-host-test-model")
      ),
      web: {
        hostname: "127.0.0.1",
      }
    ...
    const submitted = await postJson(`${app.url}/wanex/assistant/request`, {
      kind: "web.request",
      operation: "dispatchAction",
      requestId: "assistant_app_local_set_layout",
      action: {
        type: "set-layout",
        input: {
          layout: "split"
  ...
  it("executes a durable Team delivery through the shared agent host", async () => {
    ...
    if (afterReplay.kind !== "assistant.team-conversation.found") {
    ...
    if (recovered.kind !== "assistant.team-conversation.found") {
  ...
  it("executes a Team journey through the generic Web request boundary", async () => {
    ...
    const created = await postJson(`${app.url}/wanex/assistant/request`, {
      kind: "web.request",
      operation: "dispatchAction",
    ...
    const participantAdded = await postJson(`${app.url}/wanex/assistant/request`, {
      kind: "web.request",
      operation: "dispatchAction",
    ...
    for (let attempt = 0; attempt < 800; attempt += 1) {
      ...
      const reconciled = await postJson(`${app.url}/wanex/assistant/request`, {
        kind: "web.request",
  ...
  it("recovers a routed Team delivery after the submitting process exits", async () => {
    const storeDir = await tempDir("wanex-assistant-host-team-route-recovery-")
    ...
    const routed = await stagedTeam.submitRoutedMessage({
      ...
      route: {
        mode: "peer",
        outcome: "deliver",
        actorPrincipalId: "assistant-host-user",
  ...
  it("rejects unsupported attachments before Resource ingest", async () => {
    ...
    const response = await fetch(`${app.url}/wanex/assistant/attachment`, {
      method: "POST",
      headers: {
  ...
  it("stores a provider credential only in the injected trusted store", async () => {
    ...
    await expect(app.providers.listProviders()).resolves.toMatchObject({
      kind: "assistant-host.configured-provider-list",
      providers: [{
        connectionId: "openai",
        presetId: "openai",
        credentialConfigured: true,
    ...
    const replacement = "local-assistant-replacement-credential"
  ...
  it("reconciles interrupted Provider setup without deleting foreign credentials", async () => {
    const storeDir = await tempDir("wanex-assistant-host-setup-recovery-")
    const storage = createStorageTestStore({
      kind: "local-system-service",
      mode: "oneshot",
      storeDir,
  ...
  it("removes Providers with deterministic fallback and retains live credentials", async () => {
    ...
    try {
      ...
      expect(deepseekRemoved).toMatchObject({
        kind: "assistant-host.provider.removed",
  ...
  it("requires the host-session capability for browser credential setup", async () => {
    const storeDir = await tempDir("wanex-assistant-host-browser-setup-")
    const credentialStore = new TestSecretStore()
    ...
    expect(initialHtml).toContain(
      'data-provider-management-path="/wanex/assistant/providers"'
    )
    expect(initialHtml).toContain("data-app-root")
    expect(initialHtml).not.toContain(credential)

    const denied = await fetch(`${app.url}/wanex/assistant/providers`, {
      method: "POST",
    ...
    const removed = await fetch(`${app.url}/wanex/assistant/providers`, {
      method: "DELETE",
      headers: {
        "content-type": "application/json",
        "x-wanex-host-session": await readHostSessionToken(app.url)
  ...
  it("does not reflect credential-store failures through the browser setup endpoint", async () => {
    ...
    const response = await fetch(`${app.url}/wanex/assistant/providers`, {
      method: "POST",
      headers: {
        "content-type": "application/json",
        "x-wanex-host-session": await readHostSessionToken(app.url)
  ...
  it("submits a conversation through the local Web request envelope", async () => {
    ...
    const submitted = await postJson(`${app.url}/wanex/assistant/request`, {
      kind: "web.request",
      operation: "dispatchAction",
      requestId: "assistant_app_local_start_workbench",
      action: {
        type: "submit-conversation",
        input: {
          text: "hello from Assistant Host workbench"
        }
      }
    })
    ...
    expect(submitted).toMatchObject({
      ...
      actionResult: {
        ...
        snapshot: {
          ...
          view: {
            sessionCount: 1,
            selectedSessionTitle: "hello from Assistant Host workbench"
          }
        }
      }
    })
    ...
    expect(
      snapshot.web.conversation.operation?.transcript.rows.some(
        (row) =>
          ...
          "hello from Assistant Host workbench"
      )
    ).toBe(true)
  ...
  it("formats CLI startup output from the safe host snapshot", async () => {
    ...
    expect(summary).toContain(
      "Privacy: host-only details hidden from assistant snapshot"
    )
...
async function waitForLocalConversationTerminal(
  ...
  throw new Error("Assistant Host conversation did not finish")
}
...
async function waitForLocalTeamRound(
  ...
  throw new Error("Assistant Host Team discussion round did not finish")
}
...
function fakeEndpoint(
  ...
  return {
    ...
    model: {
      ...
      catalog: {
        source: "builtin",
        catalogId: `assistant-host.test.${id}`,
        revision: "1"
      }
    }
  }
