...
const SESSION_A = "ses_eval_product_side_query_a"
const SESSION_B = "ses_eval_product_side_query_b"
...
export const sideQueryScenario = createEvalScenario({
  id: "product.side-query-operational",
  title:
    "Product side query preserves selected context, binding, cancellation, and durable state",
  tags: [
    "product",
    "side-query",
    "provider-binding",
  ...
  async run(context) {
    const storeDir = await mktemp("wanex-eval-product-side-query-")
    ...
    try {
      ...
      surface = createSurfaceAdapter(app, {
        streamId: "eval-product-side-query-stream"
      })
      ...
      assert(started.ok, "Product side query should return a running receipt")
      assert(
      ...
      assert(
        ...
        "late Product selection/profile changes must not rebind the running query"
      )
      ...
      assert(
        retainedSuccess.ok &&
          retainedSuccess.value.kind === "product.side-query.found" &&
      ...
      assert(
        ...
        "exact cancel should settle the retained Product query"
      )
      ...
      assert(
        ...
        "exact terminal dismiss should remove Product presentation state"
      )
      ...
      assert(
        !disposeCompleted,
        "Product disposal must wait for Provider cleanup to drain"
      )
