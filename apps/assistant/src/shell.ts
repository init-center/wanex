...
export async function createShell(
  ...
  const commandExecutionEvents = createCommandExecutionEventHub({
    ...(options.assistantCommands?.executionInvalidations === undefined
      ? {}
      : { source: options.assistantCommands.executionInvalidations }),
  });
  ...
  const pluginManagement = createAssistantPluginManagementService({
  ...
  return {
    ...
    readAssistantCommands() {
      return backend.commands.readAssistantCommands();
    },
    async dispatchAssistantCommand(request) {
      return await dispatchCommandWithPolicy({
        backend,
        command: request,
      });
    },
    async dispatchAssistantCommandJson(body) {
      return await dispatchCommandJsonWithPolicy({
        backend,
        body,
      });
    },
    async previewAssistantCommandInvocation(request) {
      return await previewCommandInvocationWithPolicy({
        backend,
        request,
      });
    },
    async executeAssistantCommand(request) {
...
async function readHome(
  ...
  const [assistant, modelEndpoints] = await Promise.all([
    backend.commands.readAssistantOverview(options?.overview),
...
function readSettings(
  ...
  const assistant = backend.status();
  ...
  return {
    ...
    state: stateSnapshot(state),
    profile: {
      ...(assistant.activeModelEndpointId === undefined
        ? {}
        : { activeModelEndpointId: assistant.activeModelEndpointId }),
      agentContextConfigured: assistant.agentContext.configured,
      agentContextRevision: assistant.agentContext.revision,
    },
    renderer: {
      layout: state.layout,
      mode: state.mode,
...
async function readSessionTranscript(
  ...
  if (sessionId === undefined) {
    return {
      kind: "assistant.session-transcript.no-session",
...
function failedWorkbench(
  ...
  return {
    kind: "assistant.workbench.failed",
    sessionId,
