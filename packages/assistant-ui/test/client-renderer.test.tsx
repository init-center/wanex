...
describe("Web client", () => {
  ...
  it("creates a schedule from the Settings surface and keeps the assistant contract typed", async () => {
  ...
  it("requires in-assistant confirmation before removing a schedule", async () => {
    const snapshot = scheduleSnapshot();
    const actions: Action[] = [];
    let removeAttempt = 0;
    await mount(createClient(snapshot, async (action) => {
  ...
  it("opens Provider settings as a focused overlay instead of a layout column", async () => {
    ...
    expect(dialog.textContent).not.toMatch(/Assistant setup|trusted host|renderer|redacted/u);
      ...
      const output = action.type === "read-plugin-management"
        ? {
            ...
            result: ready.pluginManagement.ok
              ...
              : { kind: "assistant.plugin-management.unavailable" as const, reason: "not_configured" as const, message: "Unavailable" },
          }
        : undefined;
      return {
        ok: true,
  ...
  it("settles appearance preferences through the canonical Assistant snapshot", async () => {
    ...
    const shell = requiredElement<HTMLElement>("[data-ui-assistant-shell]");
  ...
  it("keeps canonical appearance after a rejected preference update", async () => {
    ...
    expect(requiredElement<HTMLElement>("[data-ui-assistant-shell]").dataset.theme).toBe("system");
  ...
  it("searches commands and executes generated structured input after explicit review", async () => {
    ...
    expect(actions[0]).toEqual({
      ...
      input: {
        commandId: "assistant.memory.inspect",
  ...
  it("turns the Assistant topbar into macOS integrated window chrome", () => {
  ...
  it("uploads attachment bytes through the trusted client and removes canonical drafts through Assistant", async () => {
    const initial = baseSnapshot();
  ...
  it("opens trusted Provider onboarding when required and clears the submitted credential", async () => {
    ...
    const client: Client = {
      ...
      async listProviders() {
        return { kind: "assistant-host.configured-provider-list", providers: [] };
      },
  ...
  it("dispatches regeneration as a fresh Assistant conversation operation", async () => {
    const snapshot = baseSnapshot();
    const actions: Action[] = [];
    await mount(createClient(snapshot, async (action) => {
      actions.push(action);
  ...
  it("submits Guide current through the Assistant operation without lower identities", async () => {
  ...
  it("renders a queued follow-up with its opaque Assistant operation identity", async () => {
  ...
  it("renders a selected group as a dedicated public timeline without session-only controls", async () => {
    ...
    expect(document.querySelector("[data-ui-assistant-shell]")?.getAttribute("data-ui-operation-id"))
      .toBeNull();
  });
  ...
  it("manages group participants through public Assistant identities", async () => {
    const snapshot = teamSnapshot();
    const actions: Action[] = [];
    await mount(createClient(snapshot, async (action) => {
      actions.push(action);
...
function scheduleSnapshot(): Snapshot {
  ...
  const definition = scheduleDefinition();
  const availability = {
    kind: "assistant.schedule-availability" as const,
  ...
  const schedule = {
    kind: "assistant.schedule-summary" as const,
    scheduleId: definition.scheduleId,
...
function teamSnapshot(options: {
  ...
  const team = {
    ...
    page: {
      kind: "assistant.team-conversation-page" as const,
      conversation,
      participants: [
        {
      ...
      messages: [
        ...
        },
        {
          ...
          content: [{
            ...
            text: "The storage boundary remains Assistant-owned.",
          }],
          revision: 1,
          createdAt: 6,
          updatedAt: 6,
...
function commandPaletteSnapshot(): Snapshot {
  ...
  const rows: Snapshot["view"]["commandPalette"]["rows"] = [
    ...
    },
    {
      ...
      handlerRef: "wanex.assistant.backend.memory.inspect",
      sourceKind: "builtin",
      sourceId: "wanex.assistant",
      trust: "trusted",
      category: "memory",
    ...
    },
    {
      ...
      handlerRef: "wanex.assistant.backend.unsupported",
      sourceKind: "builtin",
      sourceId: "wanex.assistant",
      trust: "trusted",
      input: {
...
function capabilitySnapshot(snapshot: Snapshot): Snapshot {
  const request = {
    kind: "assistant.capability-request",
    operation: "image.generate",
    requirements: [{
