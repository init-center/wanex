...
function guidedFollowUpAdmissionIdentity(
  ...
  const digest = createHash("sha256")
    ...
    .slice(0, 32);
  return {
    inputId: `inp_assistant_guided_${digest}`,
    turnId: `turn_assistant_guided_${digest}`,
    inputIdempotencyKey: `assistant:guided-input:${digest}`,
    jobId: `job_assistant_guided_${digest}`,
    jobIdempotencyKey: `assistant:guided-job:${digest}`,
  };
}
