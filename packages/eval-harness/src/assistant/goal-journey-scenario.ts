...
const SESSION_ID = "ses_eval_assistant_goal"
const ENDPOINT_ID = "eval-assistant-goal-provider"
const MODEL_ID = "eval-assistant-goal-model"
const SECRET_REF = "static://eval-assistant-goal-provider"
const OBJECTIVE = "Complete the bounded Assistant Goal journey"

export const goalJourneyScenario = createEvalScenario({
  id: "assistant.goal-mode-operational",
  title: "Assistant Goal Mode projects one App-owned durable Goal journey",
  tags: [
    "assistant",
    "goal",
    "surface",
  ...
  async run(context) {
    const storeDir = await mktemp("wanex-eval-assistant-goal-")
    ...
    const app = await createShell({
      ...
      secretResolver: new SecretResolver([
        new StaticSecretProvider({
          values: { [SECRET_REF]: "eval-assistant-goal-secret" }
        })
      ]),
      stateStore
    })
    ...
    const surface = createSurfaceAdapter(app, {
      streamId: "eval-assistant-goal-stream"
    })
    ...
    try {
      ...
      assert(
        web.snapshot().goal.state === "missing" &&
          initialTuiGoal.ok &&
          initialTuiGoal.value.kind === "assistant.goal.missing",
      ...
      const started = await client.startGoal({
        ...
        idempotencyKey: "eval-assistant-goal-start"
      })
      assert(
        started.ok &&
          started.value.state === "active" &&
          started.value.attemptCount === 1,
        "Assistant Surface should start one App-owned Goal and admit one attempt"
      )
      ...
      const stale = await client.pauseGoal({
        ...
        idempotencyKey: "eval-assistant-goal-stale-pause"
      })
      assert(!stale.ok, "stale Goal revision must fail closed at Assistant Surface")

      const paused = await client.pauseGoal({
        goalId,
        expectedRevision: started.value.revision,
        reason: "pause at the first review boundary",
        idempotencyKey: "eval-assistant-goal-pause"
      })
      assert(
      ...
      assert(
        canonicalPaused.ok &&
          canonicalPaused.value.kind === "assistant.goal.found" &&
      ...
      const resumed = await client.resumeGoal({
        ...
        idempotencyKey: "eval-assistant-goal-resume"
      })
      assert(
        resumed.ok &&
      ...
      assert(
        canonical.ok &&
          canonical.value.kind === "assistant.goal.found" &&
      ...
      const goalInvalidations = observedEvents.filter(
        (event) => event.type === "assistant.surface.goal.invalidated"
      )
      ...
      assert(
        webSnapshot.goal.goal?.goalId === goalId &&
          ...
          tuiSnapshot.goal.ok &&
          tuiSnapshot.goal.value.kind === "assistant.goal.found" &&
      ...
      const assistantStateJson = JSON.stringify(stateStore.snapshot())
      assert(
        !assistantStateJson.includes(goalId) &&
          !assistantStateJson.includes(OBJECTIVE) &&
          !assistantStateJson.includes("Independent verification"),
        "Assistant persistence must contain no Goal selection, content, or durable copy"
      )
      assert(
      ...
      return {
        ...
        tuiState:
          ...
          tuiSnapshot.goal.value.kind === "assistant.goal.found"
            ...
            : "unavailable",
        opaqueAssistantPersistence: !assistantStateJson.includes(goalId)
      }
    } finally {
      provider.releaseFirstVerifier()
      unsubscribe()
