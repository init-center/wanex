...
export const productAppConversationLifecycleScenario = createEvalScenario({
  ...
  async run(context) {
    ...
    try {
      ...
      const web = await createProductAppWebSurface({
      ...
      assert(
        competing.kind === "product-app.conversation-operation.rejected" &&
      ...
      const progressing = await eventually(async () => {
        const snapshot = await web.pollEvents({ limit: 50 })
        assert(
      ...
      assert(
        settled.conversation.operation?.transcript.rows.some(
          (row) =>
            row.role === "assistant" &&
            row.text === `${STREAMED_TEXT}${SETTLED_TEXT}`
        ) === true,
      ...
      const restartedWeb = await createProductAppWebSurface({
      ...
      assert(
        regeneratedTerminal.conversation.state === "succeeded" &&
          regeneratedTerminal.conversation.operation?.transcript.rows.some(
            (row) =>
              row.role === "assistant" && row.text === REGENERATED_TEXT
          ) === true,
...
type ProductAppStateStore = ReturnType<
  typeof createMemoryProductAppStateStore
>
...
function withStaleEventProbe(
  client: ProductAppSurfaceClient,
  ...
  return {
    ...
    async readSurfaceEvents(request) {
      ...
      probe.injected = true
      const staleEvent: ProductAppSurfaceEvent = {
...
async function waitForTerminal(
  web: Awaited<ReturnType<typeof createProductAppWebSurface>>
