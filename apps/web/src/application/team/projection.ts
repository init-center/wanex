...
export function projectTeamView(request: {
  ...
  if (result.kind === "product.team-conversation.found") {
    if (
  ...
  if (result.kind === "product.team-conversation.missing") {
    return {
      kind: "web.team",
      state: "missing",
      availability,
  ...
  if (result.kind === "product.team-conversation.unavailable") {
    return {
      kind: "web.team",
      state: "unavailable",
...
export async function reconcileTeamEvents(request: {
  ...
  const invalidations = request.events.ok
    ? request.events.events.filter(
        (event) => event.type === "product.surface.team.invalidated"
      )
    : []
...
> {
  return (
    ...
    (value as { readonly kind?: unknown }).kind ===
      "product.team-conversation.found" &&
