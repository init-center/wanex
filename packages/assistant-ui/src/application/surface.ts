...
function shouldRefreshPluginManagement(
  ...
  return (
    ...
    events.events.some(
      (event) =>
        event.type === "assistant.surface.plugin-management.invalidated"
    )
  )
}
...
function shouldRefreshExecutionActivity(
  ...
  return !events.ok ||
    ...
    events.events.some((event) =>
      event.type === "assistant.surface.command-execution.invalidated" &&
...
function sessionIdFromSelection(
  selection: import("@wanex/assistant").StateSnapshot["selection"]
): string | undefined {
