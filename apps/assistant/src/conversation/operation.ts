        ...
        if (state.pendingGuidedFollowUps[sessionId] !== undefined) {
          return {
            value: rejected(
              ...
              sessionId,
              active.kind === "assistant.conversation-operation.found"
                ? active.operation
                : undefined,
            ),
          };
...
export async function cancelTrackedConversationOperation(request: {
  ...
  if (sessionId === undefined) {
    return {
      kind: "assistant.conversation-operation.cancel",
      status: "untracked",
    ...
    async (state) => {
      ...
      const receipt =
        await request.backend.commands.submitConversationOperation({
          ...
          origin: {
            kind: "interactive",
            sourceRef: "assistant.regenerate",
    ...
    async (state) => {
      ...
      const receipt =
        await request.backend.commands.submitConversationOperation({
          ...
          origin: {
            kind: "interactive",
            sourceRef: "assistant.capability-continuation",
            metadata: {
