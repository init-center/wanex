...
const SESSION_ID = "ses_eval_assistant_same_turn_steering"
...
export const sameTurnSteeringScenario = createEvalScenario({
  id: "assistant.same-turn-steering-operational",
  title:
    "Assistant guides one active Turn at a Provider safe point without identity leakage",
  tags: [
    "assistant",
    "conversation",
    "steering",
  ...
  async run(context) {
    const storeDir = await mktemp("wanex-eval-assistant-same-turn-steering-")
    ...
    try {
      ...
      assert(
        ...
        "Assistant should return one steerable active conversation operation"
      )
      await withTimeout(
      ...
      assert(
        ...
        "initial Assistant submission should create one Session, Turn, and attempt"
      )
      ...
      const steered = await web.dispatchAction(
        ...
        { requestId: "eval-assistant-same-turn-steering" }
      )
      ...
      assert(
        ...
        "the same opaque Assistant operation should settle without pending steering"
      )
      assert(
        terminalOperation.transcript.rows.some(
          (row) =>
            row.role === "user" &&
            assistantConversationRowText(row) === STEERING_TEXT
        ) &&
          terminalOperation.transcript.rows.some(
            (row) =>
              row.role === "assistant" &&
              assistantConversationRowText(row) === FINAL_RESPONSE
          ),
        "canonical Assistant transcript should contain promoted guidance and the final response"
      )
      assert(
...
function assertNoLowerIdentityLeak(value: unknown, identities: readonly string[]) {
  ...
  for (const identity of identities) {
    assert(
      ...
      `Assistant/Web output exposed lower execution identity: ${identity}`
    )
  }
}
