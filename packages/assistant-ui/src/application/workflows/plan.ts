...
export function projectPlanFromResult(
  ...
  if (result.kind === "assistant.plan-generation") {
  ...
  if (result.kind === "assistant.plan-generation.missing") {
    const { generation: _discarded, ...rest } = previous
    return rest
  }
  if (result.kind === "assistant.plan-execution.submitted") {
    return {
      ...previous,
      proposal: {
...
export async function reconcilePlan(request: {
  ...
  const explicitProposalId =
    plan.generation?.state === "succeeded"
      ...
      : plan.proposal.kind === "assistant.plan-proposal.found"
