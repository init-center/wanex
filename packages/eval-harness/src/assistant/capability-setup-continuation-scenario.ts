...
import { startAssistantWebApp, type AssistantWebApp } from "@wanex/assistant-host"
import { createEvalScenario } from "../runner.js"
import { assert } from "../scenario-utils.js"
import {
  assistantConversationRowResources,
...
export const capabilitySetupContinuationScenario = createEvalScenario({
  id: "assistant.capability-setup-linked-continuation",
  title: "Assistant Local configures an optional image capability and continues one linked Turn",
  tags: ["assistant", "assistant-host", "capability", "conversation", "resource", "security"],
  async run(context) {
    const storeDir = join(context.storeDir, "assistant-capability-setup-continuation")
    ...
    try {
      ...
      assert(
        setup.kind === "assistant-host.capability-setup.continued",
      ...
      assert(
        ...
        "capability setup must not restart the Assistant Host"
      )
      ...
      const generatedResource = terminal.operation.transcript.rows
        .flatMap(assistantConversationRowResources)
      ...
      assert(
        ...
        "Assistant must correlate the real image Tool call and result as one succeeded activity"
      )
      ...
      assert(history.kind === "assistant.session-transcript.found", "Assistant history must be readable")
      const displayedUserRows = history.transcript.rows.filter((row) => row.role === "user")
      assert(
        displayedUserRows.length === 1 &&
          assistantConversationRowText(displayedUserRows[0]!) ===
            "Generate a trusted green square.",
        "Assistant history must display the original user prompt once"
      )
      assert(
