...
export async function createBackendApp(
  ...
  const commandRegistry = createBackendCommandRegistry({
    commands: {
      ...
      readAssistantWorkbench: async (request) =>
        await routerCommands.readAssistantWorkbench(request),
      readAssistantOverview: async (options) =>
        await routerCommands.readAssistantOverview(options),
      readAssistantDiagnosticsDetail: async (options) =>
        await routerCommands.readAssistantDiagnosticsDetail(options),
      shutdown: app.commands.shutdown,
    },
    ...(options.extensions?.source === undefined
      ? {}
      : { extensionCatalog: options.extensions.source }),
    ...(options.assistantCommands?.extensionExecutor === undefined
      ? {}
      : {
          extensionCommandExecutor: options.assistantCommands.extensionExecutor,
        }),
    status,
  });
  ...
  const routerCommands: BackendInputRouterCommands = {
    ...
    async readAssistantWorkbench(request) {
    ...
    async readAssistantOverview(options) {
      return await readBackendOverview(
        {
          status,
          readAssistantCapabilities: routerCommands.readAssistantCapabilities,
          readAssistantCommands: routerCommands.readAssistantCommands,
    ...
    async readAssistantDiagnosticsDetail(options) {
    ...
    readAssistantCommands: commandRegistry.readAssistantCommands,
    explainAssistantCommandContribution:
      commandRegistry.explainAssistantCommandContribution,
    previewAssistantCommandInvocation:
      commandRegistry.previewAssistantCommandInvocation,
    executeAssistantCommand: commandRegistry.executeAssistantCommand,
