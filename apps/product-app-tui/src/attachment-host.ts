...
import type { ProductAppTuiAttachmentHost } from "./types.js"

const MAX_TUI_ATTACHMENT_BYTES = 25 * 1024 * 1024

export function createProductAppTuiAttachmentHost(
  productApp: ProductAppShell
): ProductAppTuiAttachmentHost {
  return {
    async attachPath(request) {
      ...
      await productApp.prepareConversationAttachment({
