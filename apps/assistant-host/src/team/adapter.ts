...
const LOCAL_TEAM_PRINCIPAL = "assistant-host-team"
const LOCAL_USER_PRINCIPAL = "assistant-host-user"
...
function createPort(
  ...
  return {
    ...
    async listConversations(request) {
      const conversations = await listAssistantConversations(runtime, request)
      return {
    ...
    async createConversation(request) {
      ...
      const conversation = await runtime.createConversation({
        id: `team_assistant_${digest.slice(0, 32)}`,
      ...
      const user = await runtime.addParticipant({
        ...
        displayName: "You",
        role: "requester",
        idempotencyKey: `assistant-team-user:${stableDigest(conversation.id)}`
      })
      events.notify({
        conversationId: conversation.id,
    ...
    async closeConversation(request) {
      await requireOpenAssistantConversation(runtime, request.conversationId)
    ...
    async addParticipant(request) {
      const conversation = await requireOpenAssistantConversation(
        runtime,
        request.conversationId
      )
...
async function addAgentParticipant(
  ...
  return await runtime.addParticipant({
    id: `tpart_assistant_agent_${bindingDigest.slice(0, 32)}`,
    conversationId: request.conversationId,
    principalId: `assistant-host-agent-${stableDigest(request.agentSessionId).slice(0, 24)}`,
    kind: "agent",
    ...
    idempotencyKey: `assistant-team-agent:${bindingDigest}`
  })
}
...
async function submitRound(
  ...
  const digest = stableDigest(request.idempotencyKey)
  const messageId = `tmsg_assistant_${digest.slice(0, 32)}`
  ...
  const message = {
    ...
    content: [{
      type: "text" as const,
      id: `part_assistant_${digest.slice(0, 32)}`,
      text: request.text
    }]
  }
  const routed = conversation.mode === "peer"
    ...
    : await runtime.submitOrchestratedMessage({
        idempotencyKey: `assistant-team-submit:${digest}`,
        message
      })
...
async function submitDiscussionRound(
  ...
  return await runtime.submitRoutedMessage({
    idempotencyKey: `assistant-team-submit:${options.digest}`,
    ...
    route: {
      ...
      deliveries: targetIds.map((targetParticipantId) => ({
        id: `tdel_assistant_${stableDigest(`${options.message.id}\u0000${targetParticipantId}`).slice(0, 32)}`,
        targetParticipantId,
        role: "speaker",
        trigger: "round"
      }))
...
async function requireOpenAssistantConversation(
  runtime: TeamConversationRuntime,
  conversationId: string
): Promise<TeamConversationRecord> {
  const conversation = await getAssistantConversation(runtime, conversationId)
...
function localUserParticipantId(conversationId: string): string {
  return `tpart_assistant_user_${stableDigest(conversationId).slice(0, 32)}`
}
