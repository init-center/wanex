...
const PROFILE_ID = "eval-assistant-conversation-lifecycle"
const PROVIDER_BASE_URL = "https://provider.lifecycle.example.test/v1"
const SECRET_REF = "env://WANEX_EVAL_ASSISTANT_CONVERSATION_KEY"
const SECRET_VALUE = "wanex-eval-assistant-conversation-secret"
...
export const conversationLifecycleScenario = createEvalScenario({
  id: "assistant.conversation-lifecycle-operational",
  title:
    "Assistant conversation receipts, progress, cancel, regeneration, and recovery stay coherent",
  tags: [
    "assistant",
    "conversation",
    "progress",
  ...
  async run(context) {
    const storeDir = await mktemp("wanex-eval-assistant-conversation-")
    const stateStore = createMemoryStateStore()
    const provider = createProviderFixture()
    const originalFetch = globalThis.fetch
    globalThis.fetch = provider.fetch
    let firstStack: Awaited<ReturnType<typeof createAssistantStack>> | undefined
    let secondStack: Awaited<ReturnType<typeof createAssistantStack>> | undefined

    try {
      firstStack = await createAssistantStack({
        storeDir,
      ...
      const submitted = await web.dispatchAction({
        ...
        input: {
          ...
          sessionId: "ses_eval_assistant_conversation_stream"
        }
      })
      ...
      assert(
        ...
        "Assistant should return a nonterminal receipt before provider completion"
      )
      ...
      assert(
        ...
        "Assistant should reject a second same-session turn while one is active"
      )
      ...
      assert(
        ...
        "settled Assistant operation should retain its opaque identity"
      )

      await disposeAssistantStack(firstStack)
      firstStack = undefined
      secondStack = await createAssistantStack({
        storeDir,
        serviceBin: context.serviceBin,
        stateStore
      })
      ...
      assert(
        ...
        "Assistant restart should reconnect through its persisted operation reference"
      )
      ...
      const cancelSubmitted = await restartedWeb.dispatchAction({
        ...
        input: {
          ...
          sessionId: "ses_eval_assistant_conversation_cancel"
        }
      })
      ...
      assert(
        ...
        "cancellation fixture should begin as an active Assistant operation"
      )
      await withTimeout(
      ...
      assert(cancelRequested.ok, "Assistant cancellation command should return")
      ...
      assert(
        ...
        "regeneration should create a fresh opaque Assistant operation"
      )
      ...
      assert(
        regeneratedTerminal.conversation.state === "succeeded" &&
          regeneratedTerminal.conversation.operation?.transcript.rows.some(
            (row) =>
              row.role === "assistant" &&
              assistantConversationRowText(row) === REGENERATED_TEXT
          ) === true,
      ...
      if (secondStack !== undefined) {
        await disposeAssistantStack(secondStack)
      }
...
async function createAssistantStack(request: {
  ...
  const app = await createShell({
    ...
    modelEndpoint: evalOpenAICompatibleModelEndpoint({
      id: PROFILE_ID,
      modelId: "eval-assistant-conversation-model",
      baseUrl: PROVIDER_BASE_URL,
      secretRef: SECRET_REF
    }),
    secretResolver: new SecretResolver([
      new EnvSecretProvider({
        WANEX_EVAL_ASSISTANT_CONVERSATION_KEY: SECRET_VALUE
      })
    ]),
...
async function disposeAssistantStack(
  stack: Awaited<ReturnType<typeof createAssistantStack>>
): Promise<void> {
  ...
  return {
    ...
    async readSurfaceEvents(request) {
      ...
      const staleEvent: SurfaceEvent = {
        id: `assistant_app_surface_event_stale_${sequence}`,
        sequence,
        ...
        conversation: {
          ...
          sequence: 1,
          at: Date.now(),
          operationId: "assistant_app_operation_stale",
