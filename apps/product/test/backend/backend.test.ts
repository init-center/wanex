...
describe("@wanex/product backend", () => {
  it("freezes the product integration contract", () => {
    ...
    expect(BACKEND_INTEGRATION_CONTRACT.productOwnedState).toEqual([
      "selected_session",
      "panel_layout",
      "mode_routing",
      "renderer_state",
  ...
  it("dispatches application backend command port requests through safe envelopes", async () => {
    ...
    const app = await createBackendApp({
      ...
      modelEndpoint: testModelEndpoint("product.backend-port")
    })
    ...
    try {
      ...
      await expect(
        ...
        value: {
          ...
          command: "status",
          result: {
            activeModelEndpointId: "product.backend-port"
          }
        }
      })
        ...
        value: {
          ...
          rows: [
            ...
            expect.objectContaining({
              kind: "message",
              role: "assistant",
              text: "Fake response from product.backend-port-model"
            })
          ]
        }
      })
      ...
      await expect(
        ...
        value: {
          kind: "product.backend.workbench",
          sessionId: "ses_product_app_backend_command_port",
          summary: {
            inputCount: 1,
            messageCount: 2,
            latestAssistantText: "Fake response from product.backend-port-model",
  ...
  it("maps application backend command port requests to JSON responses without a transport dependency", async () => {
    ...
    const app = await createBackendApp({
      storage: settlementFixture.storage,
      modelEndpoint: testModelEndpoint("product.backend-json-mapper")
    })
    ...
    try {
      await expect(
        mapper.dispatchJson(
          JSON.stringify({
            command:
              BACKEND_COMMAND_PORT_COMMANDS.readProductCapabilities
          })
        )
      ).resolves.toMatchObject({
        status: "success",
  ...
  it("owns the application backend, command port, and JSON mapper in one local backend shell", async () => {
    ...
    const shell = await createBackendShell({
      ...
      modelEndpoint: testModelEndpoint("product.backend-backend-shell")
    })

    expect(shell.status()).toMatchObject({
      disposed: false,
      activeModelEndpointId: "product.backend-backend-shell"
    })
  ...
  it("projects a product overview read model through typed, port, and JSON backend shell paths", async () => {
    ...
    const shell = await createBackendShell({
      ...
      modelEndpoint: testModelEndpoint("product.backend-overview")
    })
    ...
    try {
      ...
      await expect(
        ...
        provider: {
          activeEndpointId: "product.backend-overview"
        },
        capabilities: {
          selectedCount: 7,
          notSelectedCount: 2,
          selectedIds: expect.arrayContaining([
            ...
            BACKEND_CAPABILITY_IDS.productCommandRegistry
          ]),
      ...
      await expect(
        shell.dispatch({
          command: BACKEND_COMMAND_PORT_COMMANDS.readProductOverview,
          input: {
            now: 7_002
          }
        })
  ...
  it("projects product diagnostics detail through typed, port, JSON, and command paths", async () => {
    ...
    const shell = await createBackendShell({
      storage: settlementFixture.storage,
      modelEndpoint: testModelEndpoint("product.backend-diagnostics-detail")
    })
    ...
    try {
      ...
      await expect(
        ...
        kind: "product.backend.diagnostics-detail",
        generatedAt: 8_101,
        summary: {
          totalCount: expect.any(Number),
          errorCount: 0,
  ...
  it("keeps storage behind product commands and rejects calls after shutdown", async () => {
    ...
    const app = await createBackendApp({
      ...
      modelEndpoint: testModelEndpoint("product.backend-commands")
    })
    ...
    expect(app.status()).toEqual({
      ...
      started: true,
      workerCount: 1,
      activeModelEndpointId: "product.backend-commands",
      agentContext: {
        configured: false,
        revision: 0
      },
    ...
    try {
      expect(app.commands.readProductCapabilities()).toMatchObject({
        ...
        capabilities: expect.arrayContaining([
          ...
          expect.objectContaining({
            id: BACKEND_CAPABILITY_IDS.productCommandRegistry,
            state: "enabled",
            ownerPackage: "@wanex/app",
          ...
          expect.objectContaining({
            ...
            state: "not_selected",
            ownerPackage: "@wanex/plugin-command-host",
    ...
    const app = await createBackendApp({
      ...
      extensions: {
        source: createStaticAppExtensionCatalogSource({
          revision: "product-capability-extension-v1",
  ...
  it("captures one catalog generation per command operation", async () => {
    ...
    const catalog = createAppExtensionCatalog(
      productCommandGeneration("product-command-generation-a", "1.0.0")
    )
    let currentCalls = 0
    const source = {
      current() {
    ...
    try {
      ...
      expect(app.commands.readProductCommands()).toMatchObject({
        extensionRevision: "product-command-generation-a",
        commands: expect.arrayContaining([
          expect.objectContaining({
            id: "plugin.dynamic",
      ...
      catalog.publish(
        productCommandGeneration("product-command-generation-b", "2.0.0")
      )
      currentCalls = 0
      expect(app.commands.readProductCommands()).toMatchObject({
        extensionRevision: "product-command-generation-b",
        commands: expect.arrayContaining([
          expect.objectContaining({
            id: "plugin.dynamic",
      ...
      const running = app.commands.executeProductCommand({
  ...
  it("routes ordinary text through the product agent command", async () => {
    ...
    const app = await createBackendApp({
      ...
      modelEndpoint: testModelEndpoint("product.backend-route-agent")
    })
  ...
  it("exposes built-in product commands as contributions and executes allow-listed handlers", async () => {
    ...
    const app = await createBackendApp({
      ...
      modelEndpoint: productTestModelEndpoint({
        endpointId: "product.backend-command-registry",
        modelId: "product.backend-command-model"
      })
    })
    ...
    try {
      ...
      expect(
        app.commands.previewProductCommandInvocation({
          commandId: "product.context.monitor.start",
          input: { intervalMs: 99 }
        })
      ).toMatchObject({
        kind: "rejected",
        ...
        value: {
          ...
          rows: [
            ...
            expect.objectContaining({
              kind: "message",
              role: "assistant",
              text: "Fake response from product.backend-command-model"
            })
          ]
        }
      })
  ...
  it("lists extension commands but fails closed for unsupported handler refs", async () => {
    ...
    const app = await createBackendApp({
      ...
      extensions: {
        source: createStaticAppExtensionCatalogSource({
          revision: "product-unsupported-handler-v1",
          snapshot: resolveAppExtensionContributions([
            {
              id: "plugin.echo",
              domain: "command",
    ...
    try {
      ...
      await expect(
        ...
        reason: "unsupported_handler_ref",
        message:
          "product command handler is not allowed: wanex.plugin-action:plugin.echo/echo?version=1.0.0"
      })
      await expect(
        app.commands.executeProductCommand({
          commandId: "missing.command"
  ...
  it("executes extension commands only through an explicitly injected executor", async () => {
    ...
    const app = await createBackendApp({
      ...
      extensions: {
        source: createStaticAppExtensionCatalogSource({
          revision: "product-command-schema-v1",
          snapshot: resolveAppExtensionContributions([
            {
              id: "plugin.echo",
              domain: "command",
  ...
  it("normalizes workflow envelopes and projects neutral provenance", async () => {
    ...
    const app = await createBackendApp({
      ...
      modelEndpoint: testModelEndpoint("product.backend-workflow-envelope")
    })
    const storage = createStorageTestStore({
      kind: "local-system-service",
      mode: "oneshot",
  ...
  it("projects guided follow-up envelopes as queue-after-current input", async () => {
    ...
    try {
      ...
      const submitted = await storage.submitSessionTurn({
        ...
        idempotencyKey: "product.backend-guided-base-input",
        jobIdempotencyKey: "product.backend-guided-base-job",
        content: [
          {
            type: "text",
            id: "guided_base_text",
        ...
        executionBinding: createTestTurnExecutionBinding(
          productTestModelEndpoint({
            endpointId: "product.backend-fake",
            modelId: "product.backend-model"
          })
        ),
        maxSteps: 1
      })
      ...
      await expect(
        ...
        result: {
          ...
          job: {
            kind: "session.turn",
            state: "ready",
            modelEndpointId: "product.backend-fake"
          }
        }
      })
  ...
  it("runs agent turns with a product-owned context profile", async () => {
    ...
    const app = await createBackendApp({
      ...
      modelEndpoint: testModelEndpoint("product.backend-context")
    })
  ...
  it("hot reloads context profile config for later product commands", async () => {
    ...
    const app = await createBackendApp({
      ...
      modelEndpoint: testModelEndpoint("product.backend-hot-context")
    })
  ...
  it("fails closed on malformed hot reload config and keeps the previous profile", async () => {
    ...
    const app = await createBackendApp({
      ...
      modelEndpoint: testModelEndpoint("product.backend-invalid-context")
    })
    const storage = createStorageTestStore({
      kind: "local-system-service",
      mode: "oneshot",
...
async function createStoreDir(): Promise<string> {
  const storeDir = await mkdtemp(join(tmpdir(), "wanex-product.backend-"))
  tempDirs.push(storeDir)
  return storeDir
}

function productCommandGeneration(revision: string, version: string) {
  return {
    revision,
    snapshot: resolveAppExtensionContributions([
      {
