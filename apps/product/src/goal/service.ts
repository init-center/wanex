  ...
  return {
    ...
    async read(request = {}) {
      ...
      if (explicitGoalId !== undefined) {
        ...
        if (view === null) {
          return { kind: "product.goal.missing", goalId: explicitGoalId }
        }
        ...
        return { kind: "product.goal.found", goal: projectGoal(view) }
      }

      const sessionId = resolveSessionId(options.state, request.sessionId)
      if (sessionId === undefined) {
        return {
          kind: "product.goal.no-session",
      ...
      if (selected === undefined) {
        return { kind: "product.goal.missing", sessionId }
      }
      const view = await options.backend.commands.readGoal({
    ...
    async resume(request) {
      ...
      return projectGoal(await options.backend.commands.resumeGoal({
        ...
        idempotencyKey:
          optionalIdentity(request.idempotencyKey, "Goal idempotencyKey") ??
          `product:goal:resume:${randomUUID()}`
      }))
    },
    async cancel(request) {
      ...
      return projectGoal(await options.backend.commands.cancelGoal({
        ...
        idempotencyKey:
          optionalIdentity(request.idempotencyKey, "Goal idempotencyKey") ??
          `product:goal:cancel:${randomUUID()}`
      }))
    },
    async dispose() {
      if (disposed) return
