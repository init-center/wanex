...
export interface SurfaceDescriptorTransportRequest {
  readonly kind: "assistant.surface-transport.request"
...
export type SurfaceTransportEnvelope<T> =
  | {
      readonly ok: true
      readonly kind: "assistant.surface-transport.response"
...
function parseSurfaceTransportRequest(input: unknown):
  ...
  if (input.kind !== "assistant.surface-transport.request") {
    return invalidSurfaceTransportRequest(
      "request.kind must be assistant.surface-transport.request",
      "unknown",
      requestId.value
    )
  }
...
function expectSurfaceTransportResponse(
  ...
  if (
    !isRecord(input) ||
    input.kind !== "assistant.surface-transport.response" ||
