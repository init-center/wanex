...
describe("Desktop proof Provider fixture", () => {
  ...
  it("distinguishes a finished response from a client-aborted held response", async () => {
    const credential = "proof-response-state-secret"
    const heldModel = "proof-held-response-model"
    const fixture = await listenDesktopProofProvider({
      credential,
      holdResponseForModel: heldModel
    })
    fixtures.push(fixture)
    const controller = new AbortController()
    const response = await fetch(`${fixture.baseUrl}/relaunch/chat/completions`, {
      method: "POST",
      headers: {
        authorization: `Bearer ${credential}`,
        "content-type": "application/json"
      ...
      body: JSON.stringify({
        ...
        messages: [{
          role: "user",
          content: "private held response prompt"
        }],
        stream: true
      }),
    ...
    await waitFor(() => fixture.responses[0]?.state === "closed_early")
    expect(fixture.responses).toEqual([{
      kind: "chat_completion",
      modelClass: "other",
      state: "closed_early"
    }])
    const retained = JSON.stringify(fixture.responses)
    expect(retained).not.toContain(credential)
    expect(retained).not.toContain(heldModel)
    expect(retained).not.toContain("private held response prompt")
  })
