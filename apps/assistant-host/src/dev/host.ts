...
export interface AssistantDemoHost {
  readonly app: Shell
...
export async function startAssistantDemoHost(
  ...
  try {
    if (options.seed) {
      const submitted = await assistantApp.shell.submitConversationOperation({
        text: options.seedText,
        sessionId: options.sessionId
      })
      if (submitted.kind !== "assistant.conversation-operation.found") {
        throw new Error(
          `seeded Assistant Host demo failed to submit conversation: ${submitted.message}`
        )
      }
      await waitForSeedConversation(assistantApp.shell, options.sessionId)

      const opened = await assistantApp.controller.dispatchAction({
          type: "open-workbench",
          input: {
            sessionId: options.sessionId
          }
      ...
      if (!opened.ok) {
        throw new Error("seeded Assistant Host demo failed to open workbench")
      }
    }
...
async function waitForSeedConversation(
  ...
  for (let attempt = 0; attempt < 200; attempt += 1) {
    ...
    if (
      ...
      if (result.operation.state !== "succeeded") {
        throw new Error(
          `seeded Assistant Host demo conversation ended as ${result.operation.state}`
        )
      }
      return
    }
  ...
  throw new Error("seeded Assistant Host demo conversation did not finish")
}

function demoHostHandle(request: {
  readonly assistantApp: AssistantWebApp
  readonly storeDir: string
  readonly cleanupDir: string | undefined
  readonly options: LocalDemoOptions
}): AssistantDemoHost {
  return {
    app: request.assistantApp.shell,
    host: request.assistantApp.host,
    url: request.assistantApp.url,
    ...
    async close() {
      await request.assistantApp.close()
