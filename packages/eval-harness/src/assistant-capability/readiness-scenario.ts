...
import { createAssistantCapabilityStoreDir } from "./helpers.js"
import { rm } from "node:fs/promises"

export const assistantCapabilityReadinessScenario = createEvalScenario({
  id: "assistant.capability-readiness-contract",
  title: "application backend exposes capability selection without widening closure",
  tags: ["assistant-path", "capability", "distribution"],
  async run(context) {
    const storeDir = await createAssistantCapabilityStoreDir(
      "wanex-eval-assistant-capability-"
    )
    const app = await createBackendApp({
      ...
      extensions: {
        source: createStaticAppExtensionCatalogSource({
          revision: "eval-assistant-capability-v1",
    ...
    try {
      const capabilities = app.commands.readAssistantCapabilities()
      const commands = app.commands.readAssistantCommands()
      ...
      const extensionPreview = app.commands.previewAssistantCommandInvocation({
        commandId: "eval.plugin.echo",
        input: {
          text: "should not execute in preview either"
        }
      })
      const rejected = await app.commands.executeAssistantCommand({
        commandId: "eval.plugin.echo",
        input: {
      ...
      assert(
        ...
        "extension command should be visible in the assistant command registry"
      )
      assert(
