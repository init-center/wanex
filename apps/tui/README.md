...
Concrete terminal surface adapter for `@wanex/product`.

This package is a leaf upper app surface. It consumes the application
renderer-side surface client and projects canonical Product state into terminal
presentation.

## Entry Contract

Use this package when a product wants a terminal surface over application.

| Use when | Avoid when |
| --- | --- |
| A TUI should call application through `@wanex/product/surface`. | A backend needs to create application storage or lifecycle. |
| You want a full-screen chat surface, compact terminal overview, dynamic Product command workflow, or injected line session. | You need Electron, Tauri, React, or HTTP rendering. |
...
part of the default `@wanex/product` closure unless a product explicitly
depends on it.
...
terminal restoration. Wanex owns only Product projection, action mapping,
...
`readSessionTranscript`, subscribes to bounded transient Product events, and
...
open or start a conversation, `Ctrl+P` to search Product's current dynamic
...
Product preview and explicit confirmation, then executes through the same
Product Surface client. Tool approval always invalidates that workflow and
opens the highest-priority focused overlay. All pickers preserve the composer
draft and commit mutations only through accepted Product commands.

`F7` never replays ambiguous Tool work automatically. Recovery presents only
Product's bounded evidence and available decisions, submits the exact visible
revision after explicit confirmation, and keeps lower execution identities out
of terminal output. Regeneration starts one fresh Product operation only after
...
the trusted host ingests bytes and prepares Product's canonical attachment
draft. The renderer then rereads safe metadata through Product Surface. It can
...
pnpm --filter @wanex/tui start preview product.agent.submit '{"text":"hello"}'
pnpm --filter @wanex/tui start execute product.status
...
`@wanex/local-host/application`, then creates the surface client through the
application message transport contract. That shared composition owns storage,
Product Shell, durable Team runtime, Team execution, and reverse-order cleanup;
...
Product and System Service lifecycle.
...
The endpoint is parsed by Runtime's canonical Provider schema before Product
...
store-derived namespace as Product Local/Desktop.

This CLI host is trusted product entry code. Renderer-like surface code still
receives only the borrowed Product Surface client and optional attachment host;
...
surface adapter. The TUI owns only canonical snapshot presentation, Product
Surface command invocation, line-session orchestration, and terminal rendering
helpers.

The package exposes only its root and `./full-screen`. Product's dynamic
