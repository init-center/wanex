...
export const surfaceMessageTransportScenario = createEvalScenario({
  ...
  title: "product surface runs through the message transport contract",
  tags: [
    "product",
    "surface",
    "surface-client",
  ...
  async run(context) {
    const storage = await createConversationSettlementFixture({
      ...
      prefix: "wanex-eval-product-message-transport-"
    })
    const app = await createShell({
      storage: storage.storage,
      modelEndpoint: evalFakeModelEndpoint(
        "eval-product-message-transport",
        "eval-product-message-transport-model"
      )
    })
    ...
    try {
      ...
      assert(
        messages.every(
          (message) =>
            isRecord(message) &&
            message.kind === "product.surface-transport.request"
        ),
      ...
      assert(
        ...
        "message transport should not change product distribution closure"
      )
