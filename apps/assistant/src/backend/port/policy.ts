...
export async function executeCommandWithPolicy(request: {
  ...
  const result = await request.backend.commands.executeAssistantCommand(
    request.command
  )
...
export async function previewCommandInvocationWithPolicy(request: {
  ...
  const preview =
    request.backend.commands.previewAssistantCommandInvocation(request.request)
  if (
...
async function commandRequiresRunnableProvider(request: {
  ...
  switch (request.command.command) {
    ...
    case BACKEND_COMMAND_PORT_COMMANDS.executeAssistantCommand:
      return await executeAssistantCommandMayRunProvider(request)
    default:
      return false
  }
}
...
async function executeAssistantCommandMayRunProvider(request: {
  readonly backend: BackendShell
  readonly command: BackendCommandPortRequest
}): Promise<boolean> {
  ...
  const explanation =
    request.backend.commands.explainAssistantCommandContribution({ commandId })
...
function parsePreviewCommandInvocationInput(input: unknown):
  ...
  if (!isRecord(input)) {
    return {
      ok: false,
      message: "previewAssistantCommandInvocation input must be an object"
    }
  }
  if (typeof input.commandId !== "string" || input.commandId.trim().length === 0) {
    return {
      ok: false,
      message:
        "previewAssistantCommandInvocation input.commandId must be a non-empty string"
    }
  }
  return {
    ok: true,
