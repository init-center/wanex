...
import {
  ...
  disposePluginTestEnvironments,
  trustedTestSubprocessHost,
} from "./execution.js"
...
afterEach(async () => {
  await disposePluginTestEnvironments()
...
describe("@wanex/assistant-plugin-host", () => {
  it("starts with an empty durable catalog and one idle claim owner", async () => {
    const handle = await createHandle("wanex-assistant-plugin-host-empty-")
    const host = await createAssistantPluginHost({
      handle,
      executionEnvironment: createPluginTestEnvironment(),
    ...
    try {
      ...
      expect(host.assistantBinding.pluginManagement).toBeUndefined()
  ...
  it("provides a composition port without an upper-app dependency", async () => {
    ...
    const composition = createAssistantPluginComposition({
    ...
    const binding = await composition.prepare({
      handle,
      executionEnvironment: createPluginTestEnvironment(),
    })

    expect(
      binding.assistantBinding.extensions.source
        .current()
  ...
  it("runs catalog, submission, worker, and activity through one store", async () => {
    ...
    const unsubscribeExecutionInvalidations = client.subscribeSurfaceEvents((event) => {
      if (event.type === "assistant.surface.command-execution.invalidated") {
    ...
    try {
      expect(host.status()).toEqual({
        kind: "assistant-plugin-host.status",
        started: false,
        disposed: false,
        activePluginCount: 1,
        commandCount: 1,
      ...
      await expect(
        client.previewAssistantCommandInvocation({
      ...
      await expect(
        client.executeAssistantCommand({
          commandId: "plugin.command-host.echo",
          input: { text: "x" }
        })
      ).resolves.toMatchObject({
      ...
      expect(executionInvalidations).toEqual([
        expect.objectContaining({
          ...
          commandExecution: {
            kind: "assistant.command-execution.invalidated",
            sequence: 1,
  ...
  it("wakes a long-idle worker after local Assistant command submission", async () => {
    const fixture = await createFixture({
      grants: [{
        pluginId: "plugin.command-host",
        version: "1.0.0",
  ...
  it("hot-switches exact versions and reconstructs the same durable revision", async () => {
    ...
    try {
      ...
      const oldExecution = await client.executeAssistantCommand({
        commandId: "plugin.command-host.echo",
      ...
      await plugin.activateInstallPlan({
        ...
        installIdempotencyKey: "assistant-command-host-install-v2",
      })
      ...
      const newExecution = await client.executeAssistantCommand({
        commandId: "plugin.command-host.echo",
      ...
      const relaunched = await createAssistantPluginHost({
        handle: fixture.handle,
        executionEnvironment: createPluginTestEnvironment(),
        principalId: "principal_assistant_command_host_relaunch",
        worker: {
          workerId: "worker_assistant_command_host_relaunch",
          leaseMs: 60_000,
          grants,
        },
      })
  ...
  it("keeps the published generation when a new execution host cannot be built", async () => {
    ...
    const fixture = await createFixture({
      ...
      createActionHost(request) {
        ...
        return trustedTestSubprocessHost(request)
      },
    })
    ...
    try {
      await expect(fixture.client.readAssistantCommands()).resolves.toMatchObject({
        ok: true,
      ...
      await fixture.plugin.activateInstallPlan({
        plan: installPlan("2.0.0"),
        installIdempotencyKey: "assistant-command-host-failing-v2",
      })
      ...
      expect(fixture.host.status().actionHostCount).toBe(2)
  ...
  it("serializes and coalesces concurrent refresh requests", async () => {
    ...
    try {
      await fixture.plugin.activateInstallPlan({
        plan: installPlan("2.0.0"),
        installIdempotencyKey: "assistant-command-host-coalesced-v2",
      })
...
async function createFixture(request: {
  readonly grants: NonNullable<
    Parameters<typeof createAssistantPluginHost>[0]["worker"]["grants"]
  >
  readonly maxAttempts?: number
  readonly createActionHost?: NonNullable<
    Parameters<typeof createAssistantPluginHost>[0]["createActionHost"]
  >
  readonly loop?: Parameters<
    typeof createAssistantPluginHost
  >[0]["worker"]["loop"]
}) {
  const handle = await createHandle("wanex-assistant-plugin-host-")
  const storage = Object.assign(
    {},
    handle.core,
    createPluginStore(handle.transport)
  ...
  await plugin.activateInstallPlan({
    plan: installPlan(),
    installIdempotencyKey: "assistant-command-host-install"
  })
  const host = await createAssistantPluginHost({
    handle,
    executionEnvironment: createPluginTestEnvironment(),
    principalId: "principal_assistant_command_host",
    ...(request.createActionHost === undefined
      ? {}
      : { createActionHost: request.createActionHost }),
    worker: {
      workerId: "worker_assistant_command_host",
      leaseMs: 60_000,
  ...
  const shell = await createShell({
    ...
    ...host.assistantBinding,
    modelEndpoint: {
      id: "assistant-command-host-provider",
      connection: {
        id: "assistant-command-host-provider",
        providerId: "fake"
      },
      protocol: { id: "fake" },
      model: {
        id: "assistant-command-host-model",
        ...
        catalog: {
          source: "builtin",
          catalogId: "assistant-command-host.fake",
          revision: "1"
        }
      }
    }
...
function activeCommandVersion(host: Awaited<ReturnType<typeof createAssistantPluginHost>>) {
  const command = host.assistantBinding.extensions.source
    .current()
