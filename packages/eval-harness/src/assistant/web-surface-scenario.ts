...
} from "@wanex/assistant-host/web-host";
import {
  entryByName,
  runJsonAudit,
...
export const webSurfaceContractScenario = createEvalScenario({
  id: "assistant.app-web-surface-contract",
  title: "Web surface consumes typed snapshots through one application shell",
  tags: ["assistant", "web", "surface", "upper-app", "assistant-path"],
  async run(context) {
    ...
    try {
      ...
      const uploaded = await postAttachment(
        `${nodeHost.url}/wanex/assistant/attachment`,
        attachmentBytes,
        "image/png",
        "eval-upload.png",
      );
      ...
      const preview = await getResourceDelivery(
        `${nodeHost.url}/wanex/assistant/resource-delivery/prepare`,
      ...
      assert(
        ...
        "Web should dispatch session selection through the Assistant client",
      );
      ...
      assert(
        ...
        "Assistant Surface should reject invalid typed action payloads",
      );
      ...
      assert(
        reconciled.ok &&
          ...
          reconciled.snapshot.conversation.historyRows.some(
            (row) =>
              row.role === "user" &&
              assistantConversationRowText(row) === "eval application web conversation",
          ),
      ...
      assert(
        ...
        "Assistant Host should serve one browser application at the root and no legacy route",
      );
      ...
      const webFootprint = entryByName(footprint, "@wanex/assistant-ui");
      ...
      assert(
        ...
        "Web should use only the Assistant surface message client",
      );
...
function isSuccessfulAttachmentUpload(
  ...
  if (
    ...
    value.upload.kind !== "assistant-host.attachment-uploaded" ||
