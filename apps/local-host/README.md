...
The `@wanex/local-host/application` entry composes the existing application
contracts into one presentation-neutral local Product lifecycle:
...
host. `@wanex/local-host/desktop-host` adapts it for desktop IPC, while the
`@wanex/tui` executable host consumes the presentation-neutral application
entry directly. None of these wrappers duplicates Product or Team composition.
...
`submit-conversation` through `/wanex/web/request`; execution still
...
Browser progress uses `GET /wanex/web/events`, an authenticated
...
reconnects reconcile once from canonical Product reads. Command-catalog events
carry only revision identity and trigger `readProductCommands()`; they never
...
`StartLocalProductHostOptions.pluginComposition` is the sole Plugin-specific
composition port. Local Host prepares it after opening Storage and before
creating the one Product Shell, injects its Product binding into that Shell,
...
The default path does not import or depend on `@wanex/plugin-command-host`.
Products that enable Plugin commands explicitly construct
`createPluginCommandComposition(...)` from that optional leaf package.
...
Use this package when a local product backend wants the complete Web application
path without copying bootstrap glue into scripts.

| Use when                                                                      | Avoid when                                                                                   |
| ----------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| A trusted local host needs to open a store and run Product locally.            | Renderer code needs a dependency; renderers should use request envelopes or surface clients. |
...
`startLocalWebApp(...)` is trusted host code. It may receive local
...
`startLocalProductHost` from `@wanex/local-host/application`. Its handle exposes
only Product Shell/Surface capabilities and explicit local resource ports; it
...
`@wanex/local-host/system-service`. This narrow boundary keeps Runtime
...
For desktop main-process code, use the `@wanex/local-host/desktop-host`
...
```ts
import { startLocalWebApp } from "@wanex/local-host";
...
For a Product onboarding flow, the trusted host can accept a credential once
...
When no runnable Provider exists, Product Local presents this Host-owned form
as blocking onboarding and marks the underlying Product shell inert. Once a
...
unblocks the refreshed Product surface in place; it does not reload or restart
the Host. Raw credentials never enter Product Web snapshots or application
actions.

Standard Provider model metadata is resolved exactly by Provider ID and model
ID from a generated, revisioned models.dev snapshot bundled with Product
...
Tool or reasoning support. Product Local never fuzzy-matches a family name.
...
previous suggestions untouched. Raw catalog documents do not enter Product Web
...
are never deleted. Raw values never enter SQLite, Product read models, browser
...
without restarting the local host.
...
native keychain, or product-owned secret store. Product read models expose
credential readiness but never the reference or value. Generic Product Web
and desktop request envelopes expose neither credential mutation nor raw
endpoint mutation.

The standalone local host loads its native keychain adapter on demand. Bundled
...
Product Local root does not statically pull that adapter into injected-host
bundles, and Product Local no longer owns a duplicate keychain subpath.

Desktop main-process host:

```ts
import { startDesktopMainHost } from "@wanex/local-host/desktop-host";
...
```bash
WANEX_LOCAL_HOST_MODEL_ENDPOINTS_JSON="$(tr -d '\n' < ./model-endpoints.json)" \
OPENAI_API_KEY=... \
pnpm --filter @wanex/local-host start
```
...
```bash
pnpm --filter @wanex/local-host demo:web
pnpm --filter @wanex/local-host demo:web -- --no-seed
pnpm --filter @wanex/local-host demo:web -- --open
```

The workspace root aliases `pnpm demo:web`,
`pnpm demo:web:seeded`, and `pnpm demo:web:blank`
...
```bash
pnpm --filter @wanex/local-host start -- \
  --profile-root ./.wanex-local-host-smoke \
  --profile-id smoke \
  --smoke
```

Smoke mode starts local application, verifies the local browser shell, typed
snapshot/action results, layout action, workbench start action, and product
privacy boundaries, prints one JSON result to stdout, then closes the host and
exits.

The CLI defaults to profile storage under `.wanex-local-host` in the
...
`--summary-format json`, or `WANEX_LOCAL_HOST_SUMMARY_FORMAT=json`, when
scripts need one parseable JSON startup summary on stdout.

Pass `--open`, or set `WANEX_LOCAL_HOST_OPEN=1`, to open the local Web
URL in the system browser after startup. This is explicit opt-in so CI and
headless scripts stay stable.

Environment variables:

- `WANEX_LOCAL_HOST_OPEN`
- `WANEX_LOCAL_HOST_SMOKE`
- `WANEX_LOCAL_HOST_SETUP_PROVIDER`
- `WANEX_LOCAL_HOST_SUMMARY_FORMAT`
- `WANEX_LOCAL_HOST_HOSTNAME`
- `WANEX_LOCAL_HOST_PORT`
- `WANEX_LOCAL_HOST_STORE_DIR`
- `WANEX_LOCAL_HOST_PROFILE_ROOT`
- `WANEX_LOCAL_HOST_PROFILE_ID`
- `WANEX_LOCAL_HOST_SERVICE_BIN`
...
- `WANEX_LOCAL_HOST_MODEL_ENDPOINTS_FILE`
- `WANEX_LOCAL_HOST_MODEL_ENDPOINTS_JSON`
- `WANEX_LOCAL_HOST_ACTIVE_MODEL_ENDPOINT_ID`
- `WANEX_STORE_DIR`
...
served Web application request endpoint or a product-owned IPC wrapper over the
...
The handle also exposes `modelEndpoints`, a trusted host facade over Product
...
models and product-owned commands, not raw endpoint mutation or secret handling.
