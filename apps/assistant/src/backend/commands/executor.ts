...
export async function executeBackendCommand(
  ...
  try {
    ...
    switch (handlerRef) {
      ...
      case BACKEND_HANDLER_REFS.readAssistantOverview:
        return completeCommand(
          request,
          handlerRef,
          await options.commands.readAssistantOverview(parseOverviewInput(request))
        )
      ...
      case BACKEND_HANDLER_REFS.readAssistantDiagnosticsDetail:
        return completeCommand(
          request,
          handlerRef,
          await options.commands.readAssistantDiagnosticsDetail(
      ...
      case BACKEND_HANDLER_REFS.readAssistantWorkbench:
        return completeCommand(
          request,
          handlerRef,
          await options.commands.readAssistantWorkbench(parseWorkbenchInput(request))
        )
