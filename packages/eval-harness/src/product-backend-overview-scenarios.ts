  ...
  async run(context) {
    ...
    const shell = await createBackendShell({
      ...
      modelEndpoint: evalFakeModelEndpoint(
        "eval-product-overview",
        "eval-product-overview-model"
      )
    })
...
function assertOverview(value: unknown, generatedAt: number): asserts value is {
  ...
  assert(value.kind === "product.backend.overview", "overview kind should match")
  ...
  assert(
    value.provider.activeEndpointId === "eval-product-overview",
