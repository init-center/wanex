...
const SESSION_ID = "ses_eval_product_recovery_review"
const ABANDON_SESSION_ID = "ses_eval_product_recovery_abandon"
const SECRET_REF = "env://WANEX_EVAL_PRODUCT_RECOVERY_KEY"
const SECRET_VALUE = "wanex-eval-product-recovery-secret"
const PROVIDER_BASE_URL = "https://provider.recovery.example.test/v1"
const TOOL_NAME = "ambiguous_product_remote"
const TOOL_CALL_ID = "call_eval_product_recovery"
...
export const recoveryReviewScenario = createEvalScenario({
  id: "product.recovery-review-operational",
  title: "Product reviews ambiguous Tool outcomes without replay or identity leakage",
  tags: [
    "product",
    "conversation",
    "tool",
  ...
  async run(context) {
    const storeDir = await mktemp("wanex-eval-product-recovery-")
    ...
    globalThis.fetch = (async () => {
      ...
      return providerCalls === 1 || providerCalls === 3
        ...
        : openAIResponse("reconciled Product response")
    ...
    tools.register({
      ...
      annotations: { title: "Ambiguous Product remote operation" },
      runtimeBinding: createToolRuntimeBinding({
        implementationId: "wanex.eval.product.ambiguous-remote",
      ...
      async invoke(invocation) {
        ...
        assert(
          ...
          "Product recovery fixture should receive the exact Tool input"
        )
        return {
          outcome: "ambiguous",
    ...
    const app = await createShell({
      ...
      modelEndpoint: evalOpenAICompatibleModelEndpoint({
        id: "eval-product-recovery-provider",
        modelId: "eval-product-recovery-model",
        baseUrl: PROVIDER_BASE_URL,
        secretRef: SECRET_REF
      }),
      secretResolver: new SecretResolver([
        new EnvSecretProvider({
          WANEX_EVAL_PRODUCT_RECOVERY_KEY: SECRET_VALUE
        })
      ]),
      runtimeContext: {
        tools,
    ...
    try {
      ...
      assert(submitted.ok, "Product recovery fixture should be admitted")

      const recovery = await waitForRecovery(web)
      const recoveryItem = recovery.conversation.operation?.recovery?.items[0]
      assert(recoveryItem !== undefined, "Product should project a recovery item")
      assert(
      ...
      assert(
        ...
        "Product should show bounded Tool risk and attempt evidence"
      )
      ...
      assert(
        regeneration.kind === "product.conversation-operation.rejected" &&
...
async function waitForRecovery(
  ...
  return await eventually(async () => {
    ...
    assert(
      ...
      "Product conversation has not reached recovery review"
    )
    return snapshot
  })
}
