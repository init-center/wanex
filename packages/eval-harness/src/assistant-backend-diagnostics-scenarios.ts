...
export const backendDiagnosticsDetailScenario = createEvalScenario({
  id: "assistant.skeleton-diagnostics-detail-contract",
  title: "App command runtime projects diagnostics detail state",
  tags: ["assistant-path", "diagnostics", "command-port", "distribution"],
  async run(context) {
    const storage = await createConversationSettlementFixture({
      serviceBin: context.serviceBin,
      prefix: "wanex-eval-assistant-diagnostics-detail-"
    })
    const shell = await createBackendShell({
      storage: storage.storage,
      modelEndpoint: evalFakeModelEndpoint(
        "eval-assistant-diagnostics-detail",
        "eval-assistant-diagnostics-detail-model"
      )
    })
    ...
    try {
      const receipt = await shell.commands.submitConversationOperation({
        ...
        sessionId: "ses_eval_assistant_diagnostics_detail"
      })
      await storage.settlements.waitForJob(receipt.jobId)

      const typed = await shell.commands.readAssistantDiagnosticsDetail({
        now: 8_201,
        diagnosticLimit: 1,
        activityLimit: 1
      })
      const port = await shell.dispatch({
        command: BACKEND_COMMAND_PORT_COMMANDS.readAssistantDiagnosticsDetail,
        input: {
          now: 8_202,
          diagnosticLimit: 2,
          activityLimit: 2
      ...
      assert(
        !rejected.ok &&
          ...
          rejected.error.message ===
            "readAssistantDiagnosticsDetail input must be an object",
...
function assertDiagnosticsDetail(
  ...
  assert(
    value.kind === "assistant.backend.diagnostics-detail",
