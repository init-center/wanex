...
export async function resolveTrackedConversationRecovery(request: {
  ...
  const payloadError = validateProductRecoveryPayload(request.input)
  ...
  const receipt = await request.backend.commands.resolveConversationOperationRecovery({
    ...
    reason,
    idempotencyKey: productRecoveryDecisionIdempotencyKey(
...
function validateProductRecoveryPayload(
...
function productRecoveryDecisionIdempotencyKey(
  operationId: string,
  input: ResolveTrackedConversationRecoveryRequest,
  reason: string
): string {
  const digest = createHash("sha256")
    ...
    .digest("hex")
  return `product:recovery:${digest}`
}
