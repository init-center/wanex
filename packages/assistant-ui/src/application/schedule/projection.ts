...
export function projectScheduleActionOutput(
  ...
  if (action.type === "read-schedule") {
    if (![
      "assistant.schedule.found",
      "assistant.schedule.missing",
      "assistant.schedule.unavailable",
  ...
  if (![
    "assistant.schedule.applied",
    "assistant.schedule.conflict",
    "assistant.schedule.rejected",
  ].includes(String(result.value.kind))) {
    return undefined
  }
  return {
