...
} from "@wanex/assistant-host/desktop-host"
...
export const assistantDesktopHostContractScenario = createEvalScenario({
  id: "assistant.app-local-desktop-host-contract",
  title: "Assistant Host desktop-host subpath handles trusted IPC envelopes",
  tags: [
    "assistant",
    "assistant-host",
    "desktop-host",
    ...
    try {
      ...
      const workbenchResponse = await host.handleRequest({
        ...
        request: {
          ...
          action: {
            ...
            input: {
              text: "hello from assistant desktop host eval"
            }
          }
        }
      })
      ...
      assert(
        firstSnapshotRecord.kind === "desktop.response" &&
          ...
          firstLocal.kind === "assistant-host.snapshot" &&
      ...
      return {
        entry: "@wanex/assistant-host/desktop-host",
