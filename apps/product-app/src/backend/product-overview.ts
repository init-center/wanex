import type {
  ...
  ProductAppBackendOverviewAction,
  ProductAppBackendOverviewCommandCategorySummary,
  ProductAppBackendOverviewCommandRow,
  ProductAppBackendOverviewRuntimeHost,
  ProductAppBackendOverviewSessionRow,
  ProductAppBackendStatus
} from "./types.js"
import type { AppDiagnosticsSnapshot } from "@wanex/app/diagnostics"
import type {
  ProductAppBackendRecentSessionsReadModel
} from "./types-read-model.js"

export interface ReadProductAppBackendOverviewHost {
  ...
  }): Promise<ProductAppBackendRecentSessionsReadModel>
}

export async function readProductAppBackendOverview(
  host: ReadProductAppBackendOverviewHost,
  options?: ProductAppBackendOverviewOptions
): Promise<ProductAppBackendOverviewReadModel> {
  const status = host.status()
  const capabilities = host.readProductCapabilities()
  const commands = host.readProductCommands()
  const [diagnostics, recentSessions] = await Promise.all([
  ...
  return {
    ...
    provider: {
      configuredProfileId: status.providerProfileId,
      activeProfileId: status.activeProviderProfileId
    },
    context: {
...
function projectDiagnosticsSummary(
  diagnostics: AppDiagnosticsSnapshot
): ProductAppBackendOverviewReadModel["diagnostics"] {
  return {
    generatedAt: diagnostics.generatedAt,
...
function projectRecentSession(
  session: ProductAppBackendRecentSessionsReadModel["rows"][number]
): ProductAppBackendOverviewSessionRow {
  return {
...
function projectRecommendedActions(options: {
  readonly diagnostics: ProductAppBackendOverviewReadModel["diagnostics"]
  readonly runtimeHost: ProductAppBackendOverviewRuntimeHost
  readonly status: ProductAppBackendStatus
  readonly recentSessions: ProductAppBackendRecentSessionsReadModel
}): readonly ProductAppBackendOverviewAction[] {
  const actions: ProductAppBackendOverviewAction[] = []
...
function summarizeCommandCategories(
  commands: ProductAppBackendCommandRegistryReadModel
): readonly ProductAppBackendOverviewCommandCategorySummary[] {
...
function projectCommand(
  command: ProductAppBackendCommandRegistryReadModel["commands"][number]
): ProductAppBackendOverviewCommandRow {
  return {
    id: command.id,
    title: command.title,
