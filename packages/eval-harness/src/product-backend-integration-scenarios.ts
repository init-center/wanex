...
export const backendIntegrationContractScenario = createEvalScenario({
  ...
  run() {
    ...
    assert(
      contract.recommendedPackage === "@wanex/product" &&
        contract.recommendedEntryPoint === "@wanex/product" &&
        contract.rendererEntryPoint === "@wanex/product/surface",
      "Wanex Product surfaces should start from the product facade"
    )
    assert(
      contract.backendDependencies.length === 1 &&
        contract.backendDependencies[0] === "@wanex/app",
      "product backend should use the typed App backend contract"
    )
